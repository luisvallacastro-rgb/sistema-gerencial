...
function renderProforma(order) {
  ...
  const firstPageHeader = `
    <header class="top"><div class="brand-panel"><img src="assets/arte-color-uniformes-logo.png?v=20260903-original-v1" alt="Arte y Color Uniformes"><time>${printValue(printDate(order.date))}</time></div><div class="order-panel"><h1>ORDEN DE PEDIDO</h1><strong class="order-number">No. ${printValue(printableOrderNumber)}</strong><p>Tipo de Factura: ${printValue(invoiceType)}</p><p>${printValue(taxPrintLegend)}</p></div></header>
