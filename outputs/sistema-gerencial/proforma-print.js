...
function renderProforma(order) {
  ...
  const firstPageHeader = `
    <header class="top"><div class="brand-panel"><div class="brand-logos"><span class="brand-logo-frame brand-logo-frame--konfi"><img src="assets/konfi-logo.png?v=20260903-original-v1" alt="KONFI"></span><span class="brand-logo-frame brand-logo-frame--ayc"><img src="assets/arte-color-uniformes-logo.png?v=20260903-original-v1" alt="Arte y Color Uniformes"></span></div><time>${printValue(printDate(order.date))}</time></div><div class="order-panel"><h1>ORDEN DE PEDIDO</h1><strong class="order-number">No. ${printValue(printableOrderNumber)}</strong><p>Tipo de Factura: ${printValue(invoiceType)}</p><p class="${order.documentType === "CE" ? "ce-document-label" : ""}">${printValue(taxPrintLegend)}</p></div></header>
    <section class="fields">
      <div class="field"><label>Vendedor:</label><strong>${printValue(order.seller)}</strong></div><div class="field"><label>Nombre Comercial:</label><strong>${printValue(data.commercialName || order.client)}</strong></div><div class="field"><label>Razon Social:</label><strong class="legal-name-with-id"><span>${printValue(data.legalName)}</span><em>ID Cliente: ${printValue(data.clientNumber)}</em></strong></div><div class="field"><label>Giro:</label><strong>${printValue(data.businessActivity)}</strong></div><div class="field"><label>Encargado/a:</label><strong>${printValue(data.contactName)}</strong></div><div class="field"><label>Telefono:</label><strong>${printValue(data.phone)}</strong></div><div class="field"><label>Direccion:</label><strong>${printValue(data.address)}</strong></div><div class="field"><label>Email:</label><strong>${printValue(data.email)}</strong></div><div class="field"><label>NIT No.:</label><strong>${printValue(data.taxId)}</strong></div><div class="field"><label>Registro No.:</label><strong>${printValue(data.registrationNumber)}</strong></div><div class="field"><label>Tipo de Contribuyente:</label><strong>${printValue(data.taxpayerType)}</strong></div><div class="field"><label>Fecha de Entrega:</label><strong>${printValue(printDate(data.deliveryDate))}</strong></div><div class="field"><label>Condiciones de Pago:</label><strong>${printValue(data.paymentTerms)}</strong></div>
    </section>`;
  const closing = `
    ...
    <section class="strategy-row">${strategies.map(([label, stored]) => `<div class="strategy-item"><span>${label}</span><i class="check">${data.strategy === stored ? "X" : ""}</i></div>`).join("")}</section>
