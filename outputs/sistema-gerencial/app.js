...
async function openBankMaintenance(accountId) {
  ...
  const render = (formOpen = false, editing = null, bulkOpen = false) => {
    ...
    dialog.querySelector("[data-bank-provision]").addEventListener("click", () => {
      ...
      const net = gross / 1.1475; const vat = net * 0.13; const incomeTax = net * 0.0175; const labor = net * 0.07;
      ...
      provisionDialog.innerHTML = `<form><header><div><span>Provisiones bancarias</span><h2>Confirmar provisión</h2><p>${chosen.length} ${chosen.length === 1 ? "movimiento seleccionado" : "movimientos seleccionados"}</p></div><button type="button" data-provision-close>×</button></header><section class="bank-provision-summary"><article><span>Total seleccionado</span><strong>${formatMoney(gross)}</strong></article><article><span>Valor neto</span><strong>${formatMoney(net)}</strong><small>Total ÷ 1.1475</small></article></section><section class="bank-provision-details"><label><span>Nombre del cliente</span><input name="customerName" required maxlength="180" autocomplete="off" placeholder="Escribe el nombre del cliente"></label><label><span>Concepto del depósito</span><select name="paymentType" required><option value="">Seleccionar concepto…</option><option>Anticipo</option><option>Abono</option><option>Cancelación de saldo</option></select></label><label><span>Vendedor para comisión</span><select name="seller" required><option value="">Seleccionar vendedor…</option>${sellerRates.map(([seller,rate]) => `<option value="${escapeHtml(seller)}" data-rate="${rate}">${escapeHtml(seller)} · ${rate*100}%</option>`).join("")}</select></label></section><section class="bank-provision-breakdown"><div><span>IVA por pagar · 13%</span><strong>${formatMoney(vat)}</strong></div><div><span>Renta por pagar · 1.75%</span><strong>${formatMoney(incomeTax)}</strong></div><div class="labor"><span>Provisión laboral · 7% del neto</span><strong>${formatMoney(labor)}</strong></div><div class="commission"><span data-provision-commission-label>Comisión del vendedor</span><strong data-provision-commission>—</strong></div><div class="total"><span>Total a provisionar</span><strong data-provision-total>${formatMoney(vat + incomeTax + labor)}</strong></div></section><p class="bank-provision-note">Esta operación registra las provisiones y no altera el saldo bancario del movimiento.</p><footer><button type="button" data-provision-close>Cancelar</button><button type="submit">Confirmar provisión</button></footer></form>`;
