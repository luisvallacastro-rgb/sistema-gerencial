...
async function openBankMaintenance(accountId) {
  ...
  const render = (formOpen = false, editing = null, bulkOpen = false) => {
    ...
    dialog.innerHTML = `<div class="bank-maintenance-shell"><header><div><span>Movimientos bancarios</span><h2>${escapeHtml(account.bank)}</h2><p>${escapeHtml(account.account)} · ${records.length} movimientos</p></div><button type="button" data-bank-close aria-label="Cerrar">×</button></header>
      ...
      <section class="bank-movements-table"><table>${bankColumnsMarkup(["Seleccionar", ...account.fields, "Acciones"])}<thead><tr><th>Provisión</th>${account.fields.map((field) => `<th>${escapeHtml(field)}</th>`).join("")}<th>Acciones</th></tr></thead><tbody>${records.map((record) => { const deposit = Number(record.data[inflowField] || 0) > 0; return `<tr class="${record.provisioned ? "is-provisioned" : ""}"><td class="bank-provision-cell"><label class="bank-row-switch" title="${record.provisioned ? "Depósito ya provisionado" : deposit ? "Seleccionar depósito" : "Solo disponible para abonos"}"><input type="checkbox" data-bank-provision-select="${escapeHtml(record.id)}" ${selectedProvisionIds.has(record.id) ? "checked" : ""} ${!deposit || record.provisioned ? "disabled" : ""}><span></span></label>${record.provisioned ? `<small>Aplicada</small>` : ""}</td>${account.fields.map((field) => `<td class="${bankFieldType(field) === "number" ? "money" : ""}">${bankMovementCell(field, record.data[field])}</td>`).join("")}<td class="bank-line-action"><div><button type="button" data-bank-record-edit="${escapeHtml(record.id)}" aria-label="Editar movimiento" title="Editar">✎</button><button type="button" class="void" data-bank-record-void="${escapeHtml(record.id)}" aria-label="Anular movimiento" title="Anular fila">×</button></div></td></tr>`; }).join("")}</tbody></table></section>
    </div>`;
    ...
    dialog.querySelector("[data-bank-provision]").addEventListener("click", () => {
      ...
      const gross = chosen.reduce((sum, record) => sum + Number(record.data[inflowField] || 0), 0);
