...
function commercialAgendaItemEvents(item) {
  ...
  return [{ id:`${item.id}-event`, date:item.date || item.startDate, startTime:item.startTime || "07:00", endTime:item.endTime || "08:00", activity:item.activity || commercialAgendaActivities[0], result:item.result || "" }];
}
