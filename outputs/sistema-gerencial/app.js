...
function printCustomerRequestSheet(request = {}) {
  ...
  printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(request.requestNumber || "Solicitud de cliente")}</title><style>body{font:14px Arial;color:#17233a;margin:36px}header{border-bottom:3px solid #27a98b;padding-bottom:14px;margin-bottom:22px}.header-top{display:flex;align-items:flex-start;justify-content:space-between;gap:32px}.brand-logo{display:block;width:150px;height:auto;object-fit:contain;margin-top:2px}h1{margin:4px 0;font-size:28px}small{color:#64748b}.meta{display:flex;gap:28px;margin-top:10px}.grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid #cbd5e1}.cell{padding:12px 14px;border-bottom:1px solid #e2e8f0;min-height:42px}.cell:nth-child(odd){border-right:1px solid #e2e8f0}.cell b{display:block;font-size:10px;text-transform:uppercase;color:#64748b;margin-bottom:5px}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:70px;margin-top:80px}.signature{border-top:1px solid #334155;padding-top:8px;text-align:center}.signature strong,.signature span{display:block}.signature span{margin-top:3px;color:#475569;font-size:12px}@media print{body{margin:18mm}}</style></head><body><header><div class="header-top"><div><small>COMERCIALIZACIÓN · SOLICITUD DE CLIENTE</small><h1>Ficha de solicitud de cliente</h1></div><img class="brand-logo" src="${escapeHtml(logoUrl)}" alt="KONFI"></div><div class="meta"><span><b>No.</b> ${escapeHtml(request.requestNumber || "Borrador")}</span><span><b>Estado:</b> ${escapeHtml(request.status || "Borrador")}</span><span><b>Solicita:</b> ${escapeHtml(requesterName)}</span></div></header><div class="grid">${rows.map(([label, value]) => `<div class="cell"><b>${escapeHtml(label)}</b>${escapeHtml(value || "—")}</div>`).join("")}</div><div class="signatures"><div class="signature"><strong>${escapeHtml(requesterName)}</strong><span>Solicitante</span></div><div class="signature"><strong>Odaliz Valencia</strong><span>Autorizado por · Gerente de Comercialización</span></div></div><script>window.onload=()=>window.print()<\/script></body></html>`);
