...
function renderCommercialAgenda() {
  const days = Object.entries(state.commercialAgenda.reduce((grouped, item) => {
    const date = item.date || "sin-fecha";
    (grouped[date] ||= []).push(item);
    return grouped;
  }, {})).sort(([dateA], [dateB]) => dateB.localeCompare(dateA));
  const dayRows = days.map(([date, items], dayIndex) => {
    const sellers = Object.entries(items.reduce((grouped, item) => {
      const seller = item.seller || "Sin vendedor";
      (grouped[seller] ||= []).push(item);
      return grouped;
    }, {})).sort(([sellerA], [sellerB]) => sellerA.localeCompare(sellerB, "es"));
    return `<details class="commercial-agenda-day" ${dayIndex === 0 ? "open" : ""}><summary><span class="commercial-agenda-day-toggle" aria-hidden="true"></span><strong>${escapeHtml(date === "sin-fecha" ? "Sin fecha" : formatDate(date))}</strong><span>${items.length} ${items.length === 1 ? "actividad" : "actividades"}</span><span>${sellers.length} ${sellers.length === 1 ? "vendedor" : "vendedores"}</span></summary><div class="commercial-agenda-day-detail">${sellers.map(([seller, sellerItems]) => `<section class="commercial-agenda-seller"><header><strong>${escapeHtml(seller)}</strong><span>${sellerItems.length} ${sellerItems.length === 1 ? "actividad" : "actividades"}</span></header><div class="commercial-agenda-table"><table><thead><tr><th>Cliente / prospecto</th><th>Actividad</th><th>Objetivo</th><th>Producto</th><th>Resultado</th><th>Acciones</th></tr></thead><tbody>${sellerItems.map((item) => `<tr><td><strong>${escapeHtml(item.prospect)}</strong></td><td>${escapeHtml(item.activity)}</td><td>${escapeHtml(item.objective || "—")}</td><td>${escapeHtml(item.product || "—")}</td><td>${escapeHtml(item.result || "Pendiente")}</td><td><button type="button" data-commercial-agenda-edit="${escapeHtml(item.id)}" title="Editar" aria-label="Editar actividad">✎</button><button class="danger" type="button" data-commercial-agenda-delete="${escapeHtml(item.id)}" title="Eliminar" aria-label="Eliminar actividad">⌫</button></td></tr>`).join("")}</tbody></table></div></section>`).join("")}</div></details>`;
  }).join("");
