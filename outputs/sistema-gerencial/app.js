...
function quotationDraftFromForm() {
  ...
  const vatCents = Math.round(subtotalCents * .13);
...
async function openQuotationDialog(opportunityId, quoteId = "") {
  ...
  renderQuotationHistory(opportunity.id); populateQuotationForm(quote, opportunity);
...
function printQuotation(quote) {
  ...
  const rows = quote.lines.map((line) => {
    const description = [line.description, line.notes].filter(Boolean).map(value).join(". ");
    const size = line.size ? `<b class="size">Talla: ${value(line.size)}</b>` : "";
    return `<tr><td class="qty">${value(line.quantity)}</td><td>${description}${size}</td><td class="money">${formatControlSalesMoney(line.unitPriceCents)}</td><td class="money">${formatControlSalesMoney(line.lineTotalCents)}</td></tr>`;
  }).join("");
  ...
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${value(quote.number || "Cotización")}</title><style>
    @page{size:Letter;margin:0}*{box-sizing:border-box}html{background:#dfe5ec}body{margin:0;color:#111;font:12px Arial,Helvetica,sans-serif}.sheet{position:relative;width:216mm;min-height:279mm;margin:12px auto;background:#fff;padding:12mm 17mm 10mm;overflow:hidden}.letterhead{height:33mm;display:flex;justify-content:flex-end;align-items:flex-start}.logo-crop{position:relative;width:77mm;height:24mm;overflow:hidden}.logo-crop img{position:absolute;height:24mm;width:auto;right:0;top:0}.city-date{text-align:right;margin:-3mm 8mm 10mm 0;font-size:13px}.recipient{margin:0 0 8mm}.recipient strong{display:block;font-size:14px;margin-bottom:3px}.recipient span{display:block;font-size:13px}.intro{font-size:13px;margin:0 0 2mm}.quote-number{position:absolute;left:17mm;top:13mm;color:#17794f;font-weight:800;letter-spacing:.08em}.quote-number small{display:block;color:#667085;font-size:9px;text-transform:uppercase;letter-spacing:.14em;margin-bottom:3px}.quote-table{width:100%;border-collapse:collapse;table-layout:fixed}.quote-table th,.quote-table td{border:1px solid #111}.quote-table th{padding:6px 5px;background:#bdbdbd;text-align:center;font-weight:800}.quote-table th:nth-child(1){width:9%}.quote-table th:nth-child(3){width:17%}.quote-table th:nth-child(4){width:16%}.quote-table td{padding:4px 7px;vertical-align:middle;line-height:1.22}.quote-table .qty{text-align:center;font-weight:700}.quote-table .money{text-align:right;white-space:nowrap}.quote-table .size{display:inline;margin-left:4px}.quote-table .filler td{height:7mm}.quote-table tfoot td{height:8mm;font-weight:700}.quote-table tfoot td:first-child{border-left-color:transparent;border-bottom-color:transparent}.quote-table tfoot td:nth-child(2){text-align:right}.quote-table tfoot tr:last-child td{font-size:13px;font-weight:900}.guarantee{font-size:12px;font-weight:800;margin:0 0 8mm}.terms{font-size:12px;line-height:1.55}.terms p{margin:2px 0}.terms mark{background:#fff200;padding:1px 2px;font-weight:800}.signature{margin-top:12mm;line-height:1.45}.signature .closing-word{margin-bottom:7mm}.signature strong,.signature span,.signature a{display:block}.signature a{color:#0645d6}.footer-brand{position:absolute;left:17mm;right:17mm;bottom:8mm;display:grid;grid-template-columns:1fr auto 1fr;align-items:end;gap:8mm;color:#626b77;font-size:9px}.footer-contact{display:grid;grid-template-columns:1fr 1fr;gap:4px 12px}.footer-contact span{white-space:nowrap}.footer-konfi{display:flex;align-items:center;gap:5px;white-space:nowrap}.konfi-crop{position:relative;width:26mm;height:9mm;overflow:hidden}.konfi-crop img{position:absolute;height:9mm;width:auto;left:0;top:0}.motto{text-align:right;text-transform:uppercase;letter-spacing:.08em}.actions{position:fixed;z-index:10;top:14px;right:14px;display:flex;gap:8px;background:#182d4e;padding:8px;border-radius:12px;box-shadow:0 12px 30px #182d4e44}.actions button,.actions a{border:0;border-radius:8px;padding:10px 13px;background:#238760;color:white;text-decoration:none;font-weight:800;cursor:pointer}.actions button:last-child{background:white;color:#18233d}@media(max-width:850px){.sheet{margin:0;transform-origin:top left}.actions{position:sticky;justify-content:center;border-radius:0}}@media print{html,body{background:#fff}.actions{display:none}.sheet{margin:0;width:216mm;height:279mm;min-height:279mm;box-shadow:none}.quote-table thead{display:table-header-group}.quote-table tr{break-inside:avoid}.footer-brand{position:absolute}}
  </style></head><body><main class="sheet"><div class="quote-number"><small>Cotización</small>${value(quote.number || "BORRADOR")}</div><header class="letterhead"><div class="logo-crop"><img src="${value(brandUrl)}" alt="Arte y Color Uniformes"></div></header><p class="city-date">San Salvador, ${value(longDate)}</p><section class="recipient"><strong>${value(String(quote.client || "Cliente").toUpperCase())}</strong><span>${data.contactName ? `Atención: ${value(data.contactName)}` : "Presente"}</span></section><p class="intro">En atención a su solicitud y de la manera más atenta le presentamos la siguiente cotización:</p><table class="quote-table"><thead><tr><th>CANT.</th><th>DESCRIPCIÓN</th><th>PRECIO<br>UNITARIO</th><th>TOTAL</th></tr></thead><tbody>${rows}${fillerRows}</tbody><tfoot><tr><td colspan="2" rowspan="3"></td><td>SUBTOTAL</td><td class="money">${formatControlSalesMoney(quote.subtotalCents)}</td></tr><tr><td>IVA</td><td class="money">${formatControlSalesMoney(quote.vatCents)}</td></tr><tr><td>TOTAL</td><td class="money">${formatControlSalesMoney(quote.totalCents)}</td></tr></tfoot></table><p class="guarantee">*${value(quote.warrantyNote || "Todos nuestros productos están garantizados y elaborados con altos estándares de calidad.")}</p><section class="terms"><p><mark>${value(quote.commercialNotes || "Precios unitarios no incluyen IVA")}</mark></p><p><b>Vigencia de oferta:</b> ${value(quote.validDays || 30)} días</p><p><b>Tiempo de entrega:</b> ${value(quote.deliveryTerms)}</p><p><b>Forma de Pago:</b> ${value(quote.paymentTerms)}</p><p><mark>${value(quote.specialSizesNote)}</mark></p></section><section class="signature"><p class="closing-word">Atentamente,</p><strong>${value(quote.seller)}</strong><span>${value(data.sellerRole || "Ejecutivo/a de ventas")}</span>${data.sellerPhone ? `<span>${value(data.sellerPhone)}</span>` : ""}${data.sellerEmail ? `<a href="mailto:${value(data.sellerEmail)}">${value(data.sellerEmail)}</a>` : ""}</section><footer class="footer-brand"><div class="footer-contact"><span>Arte y Color Uniformes</span><span>+503 2277-2032</span><span>arteycolor.bordados@gmail.com</span><span>+503 7202-8137</span></div><div class="footer-konfi"><span>Una marca</span><div class="konfi-crop"><img src="${value(brandUrl)}" alt="KONFI"></div></div><div class="motto">Innovación, calidad y responsabilidad garantizada</div></footer></main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button><a href="${value(emailHref)}">Preparar correo</a><button onclick="window.close()">Cerrar</button></nav></body></html>`);
