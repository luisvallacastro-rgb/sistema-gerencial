...
function ensureQuotationDialog() {
  ...
  document.body.insertAdjacentHTML("beforeend", `<dialog id="quotationDialog" class="wide-dialog quotation-dialog"><form id="quotationForm">
    ...
    <section class="quotation-form-grid quotation-main-fields"><label>Número de cotización<input id="quotationNumber" readonly></label><label>Fecha<input id="quotationDate" type="date" required></label><label>Vigencia<div class="quotation-validity-field"><input id="quotationValidDays" type="number" min="1" max="365" value="30" required><span>días</span></div></label><label>Estado<select id="quotationStatus"><option>Borrador</option><option>Enviada</option><option>Aprobada</option><option>Rechazada</option><option>Vencida</option><option value="Convertida" disabled>Convertida (pedido creado)</option></select></label></section>
