...
async function printBankAvailabilityReport() {
  ...
  const row = (label, key, className = "", expandable = false) => `<tr><th>${expandable ? `<button class="expand-btn" type="button" data-toggle-detail="${key}" aria-expanded="false">＋</button>` : ""}${label}</th>${accounts.map((item) => `<td class="${className}">${key === "date" ? (item.date ? formatDate(item.date) : "—") : formatMoney(item[key] || 0)}</td>`).join("")}</tr>`;
  const status = accounts.every((item) => item.reconciled) ? "Conciliación verificada" : "Revisar diferencias";
  const movementDetail = (type) => {
    const amountKey = type === "charges" ? "charge" : "credit";
    const movements = accounts.flatMap((account) => (account.movements || []).filter((movement) => Number(movement[amountKey] || 0) > 0).map((movement) => ({ ...movement, bank:account.bank, account:account.account })));
    const movementRows = movements.map((movement, index) => `<tr><td>${index + 1}</td><td><strong>${escapeHtml(movement.bank)}</strong><small>${escapeHtml(movement.account)}</small></td><td>${movement.date ? formatDate(movement.date) : "—"}</td><td>${escapeHtml(movement.description || "Movimiento bancario")}</td><td>${escapeHtml(movement.reference || "—")}</td><td class="${type === "charges" ? "charge" : "credit"}">${formatMoney(movement[amountKey] || 0)}</td><td class="money">${formatMoney(movement.balance || 0)}</td></tr>`).join("");
    const total = type === "charges" ? report.chargesTotal : report.creditsTotal;
    return `<tr class="detail-row" data-detail-row="${type}" hidden><td colspan="${accounts.length + 2}"><div class="inline-detail"><h2>Detalle de ${type === "charges" ? "cargos" : "abonos"}</h2><table class="detail-table"><thead><tr><th>#</th><th>Banco / Cuenta</th><th>Fecha</th><th>Descripción</th><th>Referencia</th><th>${type === "charges" ? "Cargo" : "Abono"}</th><th>Saldo resultante</th></tr></thead><tbody>${movementRows || `<tr><td colspan="7">No existen ${type === "charges" ? "cargos" : "abonos"} en este corte.</td></tr>`}</tbody><tfoot><tr><th colspan="5">Total detallado</th><th>${formatMoney(total || 0)}</th><th></th></tr></tfoot></table></div></td></tr>`;
  };
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte de disponibilidad bancaria</title><style>
    @page{size:landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17233a;font:12px Arial,sans-serif;background:#eef2f7}.sheet{width:100%;max-width:1400px;margin:20px auto;padding:28px;background:#fff}header{display:flex;justify-content:space-between;align-items:end;border-bottom:3px solid #28a98c;padding-bottom:14px;margin-bottom:22px}h1{margin:3px 0;font-size:27px}header p{margin:0;color:#64748b}.status{color:#147d69;font-weight:700}.matrix,.detail-table{width:100%;border-collapse:collapse;table-layout:fixed}.matrix th,.matrix td,.detail-table th,.detail-table td{border:1px solid #cbd5e1;padding:10px 8px;text-align:right}.matrix thead th,.detail-table thead th{background:#172f51;color:#fff;text-align:center}.matrix thead th:first-child{width:170px}.matrix thead strong,.matrix thead small,.detail-table td small{display:block}.matrix thead small,.detail-table td small{margin-top:3px;color:#64748b;font-size:9px}.matrix thead small{color:#cbd5e1}.matrix tbody th{text-align:left;background:#f8fafc}.expand-btn{width:24px;height:24px;margin-right:7px;padding:0;border:1px solid #46688f;border-radius:6px;background:#e8eef6;color:#173b62;font-weight:900;cursor:pointer}.expand-btn:hover{border-color:#28a98c;color:#087f5b}.money{font-weight:700}.credit{color:#087f5b}.charge{color:#b42318}.detail-row>td{padding:0!important;text-align:left!important;background:#f8fafc}.inline-detail{padding:14px 16px 18px;border-left:4px solid #28a98c}.inline-detail h2{margin:0 0 10px;font-size:16px}.detail-table{font-size:10px}.detail-table th:nth-child(1){width:4%}.detail-table th:nth-child(2){width:15%}.detail-table th:nth-child(3){width:10%}.detail-table th:nth-child(4){width:31%}.detail-table th:nth-child(5){width:14%}.detail-table th:nth-child(n+6){width:13%}.detail-table td:nth-child(-n+5){text-align:left}.detail-table tfoot th,.detail-table tfoot td{background:#edf5f3;font-weight:800}.matrix tfoot th,.matrix tfoot td{background:#e8f5f1;font-size:14px;font-weight:800}.formula{margin:16px 0 0;color:#475569}.actions{position:fixed;right:18px;bottom:18px;display:flex;gap:8px}.actions button{padding:11px 15px;border:0;border-radius:8px;background:#173b62;color:#fff;font-weight:700}.actions button:first-child{background:#16866d}@media print{body{background:#fff}.sheet{margin:0;padding:0;max-width:none}.actions{display:none}.expand-btn{display:none}.detail-row:not([hidden]){display:table-row}}
  </style></head><body><main class="sheet"><header><div><p>FINANCIERA · DISPONIBILIDAD BANCARIA</p><h1>Reporte de actualización de saldos</h1><p>Generado ${escapeHtml(new Date().toLocaleString("es-SV"))}</p></div><div class="status">${escapeHtml(status)}</div></header><table class="matrix"><thead><tr><th>Concepto</th>${headings}<th>Total consolidado</th></tr></thead><tbody>${row("Fecha de actualización", "date")} ${row("Saldo anterior", "previousBalance", "money")} ${row("(−) Cargos", "charges", "money charge", true)}${movementDetail("charges")}${row("(+) Abonos", "credits", "money credit", true)}${movementDetail("credits")}</tbody><tfoot><tr><th>Saldo nuevo</th>${accounts.map((item) => `<td>${formatMoney(item.newBalance || 0)}</td>`).join("")}<td>${formatMoney(report.newTotal || 0)}</td></tr></tfoot></table><p class="formula"><b>Comprobación:</b> saldo anterior − cargos + abonos = saldo nuevo. Totales del corte: ${formatMoney(report.previousTotal || 0)} − ${formatMoney(report.chargesTotal || 0)} + ${formatMoney(report.creditsTotal || 0)} = <b>${formatMoney(report.newTotal || 0)}</b>.</p></main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button><button onclick="window.close()">Cerrar</button></nav><script>document.querySelectorAll('[data-toggle-detail]').forEach(function(button){button.addEventListener('click',function(){var row=document.querySelector('[data-detail-row="'+button.dataset.toggleDetail+'"]');var opening=row.hasAttribute('hidden');if(opening){row.removeAttribute('hidden');button.textContent='−';}else{row.setAttribute('hidden','');button.textContent='＋';}button.setAttribute('aria-expanded',String(opening));});});<\/script></body></html>`);
  popup.document.close();
}
