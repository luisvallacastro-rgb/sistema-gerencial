...
function collectControlSalesProformaData() {
  ...
  data.customerId = document.querySelector("#controlSalesCustomerId")?.value.trim() || "";
  data.clientNumber = document.querySelector("#controlSalesClientNumber")?.value.trim() || data.customerCode || "";
...
function fillControlSalesProformaData(data = {}, order = null) {
  ...
  const customerId = document.querySelector("#controlSalesCustomerId");
  const clientNumber = document.querySelector("#controlSalesClientNumber");
  if (customerId) customerId.value = data.customerId || order?.customerId || order?.proformaData?.customerId || "";
  if (clientNumber) clientNumber.value = data.clientNumber || data.customerCode || order?.proformaData?.clientNumber || "";
...
function quotationSellerContact(sellerName = "") {
  const seller = crmMasterSalesUsers({ includeInactive: true }).find((item) => (
    normalizeKey(item.name) === normalizeKey(sellerName)
  )) || {};
...
function wireQuotationsModule() {
  ...
  opportunityTable.querySelectorAll("[data-quotation-module-detail]").forEach((button) => button.addEventListener("click", () => (
    openQuotationDialog(button.dataset.opportunityId, button.dataset.quotationModuleDetail, null, null, false, true)
  )));
  opportunityTable.querySelectorAll("[data-quotation-module-document]").forEach((button) => button.addEventListener("click", () => {
    const quotation = state.quotations.find((item) => String(item.id) === String(button.dataset.quotationModuleDocument));
...
const customerCommercialStrategies = Object.freeze(["Retención", "Expansión", "Atracción", "Recuperación"]);

function customerSelectOptions(values, placeholder) {
...
function ensureQuotationDialog() {
  ...
  dialog.addEventListener("click", async (event) => {
    ...
    if (event.target.matches("[data-quotation-edit]")) {
      ...
      status.dataset.tone = "success";
      status.classList.remove("hidden");
      const firstField = document.querySelector("[data-quotation-description]");
      firstField?.focus();
      firstField?.select();
    }
  ...
  dialog.querySelector("#quotationDocumentType").addEventListener("change", (event) => {
    document.querySelector("#quotationCommercialNotes").value = event.target.value === "CCF"
      ? "Precios unitarios no incluyen IVA"
      : "Los precios unitarios ya incluyen IVA";
    updateQuotationTotals();
  });
...
function applyMasterCustomerToQuotation(customer) {
  ...
  const fields = {
    ...
    quotationDocumentType:data.documentType, quotationPrintObservation:data.printObservation
  };
...
function quotationDraftFromForm() {
  ...
  const masterCustomer = crmMasterCustomers(true).find((customer) => String(customer.id) === String(customerId));
  const clientNumber = masterCustomer?.clientNumber || masterCustomer?.customerCode || document.querySelector("#quotationCustomerCode").value.trim();
  return { id:document.querySelector("#quotationId").value, opportunityId:document.querySelector("#quotationOpportunityId").value, customerId, number:document.querySelector("#quotationNumber").value, date:document.querySelector("#quotationDate").value, validDays:Number(document.querySelector("#quotationValidDays").value || 30), seller:document.querySelector("#quotationSeller").value.trim(), client:document.querySelector("#quotationCommercialName").value.trim(), status:document.querySelector("#quotationStatus").value,
    customerData:{ customerId, clientNumber, commercialName:document.querySelector("#quotationCommercialName").value.trim(), legalName:document.querySelector("#quotationLegalName").value.trim(), contactName:document.querySelector("#quotationContactName").value.trim(), phone:document.querySelector("#quotationPhone").value.trim(), email:document.querySelector("#quotationEmail").value.trim(), address:document.querySelector("#quotationAddress").value.trim(), businessActivity:document.querySelector("#quotationBusinessActivity").value.trim(), taxId:document.querySelector("#quotationTaxId").value.trim(), registrationNumber:document.querySelector("#quotationRegistrationNumber").value.trim(), taxpayerType:document.querySelector("#quotationTaxpayerType").value.trim(), customerCode:document.querySelector("#quotationCustomerCode").value.trim() || clientNumber, strategy:document.querySelector("#quotationStrategy").value.trim(), clientType:document.querySelector("#quotationClientType").value.trim(), department:document.querySelector("#quotationDepartment").value.trim(), municipality:document.querySelector("#quotationMunicipality").value.trim(), documentType, printObservation:document.querySelector("#quotationPrintObservation").value.trim(), sellerPhone:document.querySelector("#quotationSellerPhone").value.trim(), sellerEmail:document.querySelector("#quotationSellerEmail").value.trim(), sellerRole:"Asesor Comercial" },
    documentType, printObservation:document.querySelector("#quotationPrintObservation").value.trim(), paymentTerms:document.querySelector("#quotationPaymentTerms").value.trim(), deliveryTerms:document.querySelector("#quotationDeliveryTerms").value.trim(), warrantyNote:document.querySelector("#quotationWarrantyNote").value.trim(), commercialNotes:documentType === "CCF" ? "Precios unitarios no incluyen IVA" : "Los precios unitarios ya incluyen IVA", specialSizesNote:document.querySelector("#quotationSpecialSizesNote").value.trim(), applyVat:documentType === "CCF", subtotalCents, vatCents, totalCents:subtotalCents + vatCents, lines, updatedBy:state.currentUser?.name || "Sistema Gerencial" };
}
...
function updateQuotationTotals() {
  ...
  document.querySelector("#quotationSubtotalCard").classList.toggle("hidden", draft.documentType !== "CCF");
  document.querySelector("#quotationVatCard").classList.toggle("hidden", draft.documentType !== "CCF");
  const referenceCents = Number(document.querySelector("#quotationReference")?.dataset.referenceCents || 0);
  const pendingBalanceCents = referenceCents - draft.totalCents;
  ...
  const variation = draft.totalCents <= 0 ? "neutral" : pendingBalanceCents > 0 ? "positive" : pendingBalanceCents < 0 ? "negative" : "neutral";
  if (variationCard) variationCard.dataset.variation = variation;
  if (variationLabel) variationLabel.textContent = draft.totalCents <= 0 ? "Saldo pendiente por cotizar" : pendingBalanceCents > 0 ? "Saldo pendiente por cotizar" : pendingBalanceCents < 0 ? "Excedente sobre la oportunidad" : "Monto original cubierto";
  if (variationValue) variationValue.textContent = draft.totalCents <= 0 ? formatControlSalesMoney(referenceCents) : formatControlSalesMoney(Math.abs(pendingBalanceCents));
}
...
function populateQuotationForm(quote, opportunity = null, customerOverride = null) {
  ...
  const assignedSeller = opportunity?.seller || seller.name || quote?.seller || state.currentUser?.name || "";
  const sellerContact = quotationSellerContact(assignedSeller);
  const values = {
...
function setQuotationDialogReadOnly(dialog, readOnly) {
  if (!dialog) return;
  dialog.dataset.readOnly = readOnly ? "true" : "false";
  ...
  dialog.querySelector('button[type="submit"]')?.classList.toggle("hidden", readOnly);
  dialog.querySelector("[data-quotation-new]")?.classList.toggle("hidden", readOnly || dialog.dataset.directOrderFlow === "true");
...
async function openQuotationDialog(opportunityId, quoteId = "", opportunityOverride = null, customerOverride = null, directOrderFlow = false, readOnly = false) {
  ...
  dialog.querySelector('button[type="submit"]')?.classList.remove("hidden");
  dialog.querySelector("[data-quotation-new]")?.classList.toggle("hidden", directOrderFlow);
  setQuotationDialogReadOnly(dialog, readOnly);
...
async function saveQuotationFromForm(forcedStatus = "", openPreview = false) {
  ...
  try {
    ...
    if (linkedOpportunity && saved.client) linkedOpportunity.company = saved.client;
    ...
    if (resultOpportunity) {
      if (saved.client) resultOpportunity.company = saved.client;
      if (Number(saved.totalCents || 0) > 0) resultOpportunity.amount = Number(saved.totalCents) / 100;
      localStorage.setItem(opportunitiesStorageKey, JSON.stringify(getOpportunitySubmenu().items));
    }
...
function printQuotation(quote) {
  ...
  const sellerContact = quotationSellerContact(responsibleSeller);
  const data = { ...(quote.customerData || {}), sellerPhone:sellerContact.phone, sellerEmail:sellerContact.email };
  ...
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Cotización</title><style>
    ...
    .signature{min-height:24mm;align-content:start}
    .footer-brand{position:static;margin-top:8mm;padding-top:5mm;border-top:1px solid #d5dae1;row-gap:4mm}
  ...
  if (quotationTotalsBody) quotationTotalsBody.innerHTML = quotationTotalsRows;
  const quotationLegend = popup.document.querySelector(".terms p:first-child strong");
  if (quotationLegend) quotationLegend.textContent = quotationCommercialLegend;
  const quotationObservation = String(quote.printObservation || data.printObservation || "").trim();
  const quotationTerms = popup.document.querySelector(".terms");
  if (quotationTerms && quotationObservation) {
    const observationLine = popup.document.createElement("p");
    const observationLabel = popup.document.createElement("b");
    observationLabel.textContent = "Observaciones: ";
    observationLine.append(observationLabel, popup.document.createTextNode(quotationObservation));
    quotationTerms.insertBefore(observationLine, quotationTerms.children[1] || null);
  }
  popup.document.close();
...
function printControlSalesProformaInline(order, options = {}) {
  ...
  const brandUrl = new URL("assets/arte-color-uniformes-logo.png?v=20260903-original-v1", window.location.href).href;
  const konfiBrandUrl = new URL("assets/konfi-logo.png?v=20260903-original-v1", window.location.href).href;
...
<style>
  ...
  <section class="fields">
    ...
    <div class="field"><label>Razon Social:</label><strong class="legal-name-with-id"><span>${value(data.legalName)}</span><em>ID Cliente: ${value(data.clientNumber)}</em></strong></div>
  ...
  dialog.innerHTML = `<form class="dialog-card crm-customer-glass" id="crmCustomerForm">
    ...
    <div class="crm-customer-dialog-body">
      ...
      <section class="crm-customer-form-section"><div class="crm-customer-section-title"><span>03</span><div><strong>Información fiscal</strong><small>Identificación para cotizaciones y documentación.</small></div></div><div class="crm-customer-fields">
        ...
        <label>Tipo de factura<select id="crmCustomerDocumentType"><option value="CF">Consumidor final</option><option value="CCF">Crédito fiscal</option><option value="CE">Comprobante de Envío CE</option></select></label>
      ...
      <section class="crm-customer-form-section"><div class="crm-customer-section-title"><span>04</span><div><strong>Operación comercial</strong><small>Información reutilizable en órdenes y pedidos.</small></div></div><div class="crm-customer-fields">
        ...
        <label class="span-2">Estrategia comercial<select id="crmCustomerStrategy">${customerSelectOptions(customerCommercialStrategies, "Seleccionar estrategia comercial")}</select></label>
      </div></section>
    </div>
...
function isOdalizValenciaUser(user = state.currentUser) {
...
function ensureCustomerRequestDialog() {
  ...
  dialog.innerHTML = `<form class="dialog-card crm-customer-glass" id="customerRequestForm">
    <header class="crm-customer-dialog-head"><div><p class="eyebrow">Solicitud comercial</p><h3 data-customer-request-title>Nueva solicitud de cliente</h3><p>Guarda temporalmente las correcciones y valida únicamente cuando la revisión esté completa.</p></div><button type="button" class="crm-customer-close" data-customer-request-close aria-label="Cerrar"><svg viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18"/></svg></button></header>
    <input type="hidden" id="customerRequestId"><div class="crm-customer-dialog-body">
      ...
      <section class="crm-customer-form-section"><div class="crm-customer-section-title"><span>03</span><div><strong>Información fiscal</strong><small>Datos para validación y documentación.</small></div></div><div class="crm-customer-fields"><label>Tipo de documento<select id="customerRequestIdentityDocumentType">${customerSelectOptions(customerIdentityDocumentTypes, "Seleccionar tipo de documento")}</select></label><label>Número de documento<input id="customerRequestTaxId"></label><label>NRC / registro<input id="customerRequestRegistration"></label><label>Tipo de contribuyente<select id="customerRequestTaxpayerType">${customerSelectOptions(customerTaxpayerTypes, "Seleccionar tipo de contribuyente")}</select></label><label>Tipo de factura<select id="customerRequestDocumentType"><option value="CF">Consumidor final</option><option value="CCF">Crédito fiscal</option><option value="CE">Comprobante de Envío CE</option></select></label><label class="span-2">Giro / actividad económica<input id="customerRequestBusiness"></label></div></section>
  ...
  dialog.querySelector("[data-customer-request-sign]").addEventListener("click", async (event) => {
    if (!dialog.currentRequest?.id || !confirm("¿Autorizar la firma electrónica de esta solicitud como Odaliz Valencia?")) return;
    ...
    try {
      ...
      dialog.querySelector("[data-customer-request-status]").innerHTML = "<i></i> Firmada electrónicamente · Lista para aprobar y crear cliente";
    } catch (error) {
      button.textContent = "Autorizar firma electrónica";
  ...
  dialog.querySelector("[data-customer-request-draft]").addEventListener("click", async (event) => {
    ...
    try {
      ...
      dialog.querySelector("[data-customer-request-print]").classList.toggle("hidden", !savedId);
      dialog.querySelector("[data-customer-request-print]").textContent = "Imprimir solicitud";
  ...
  dialog.querySelector("[data-customer-request-review-save]").addEventListener("click", async (event) => {
    ...
    try {
      ...
      dialog.querySelector("[data-customer-request-status]").innerHTML = "<i></i> Pendiente · Guardado temporal completado · Firma conservada";
      button.textContent = "Guardado temporalmente";
    } catch (error) {
      button.textContent = "Guardar temporalmente";
      alert(error.message || "No fue posible guardar temporalmente la solicitud.");
    } finally { button.disabled = false; }
  });
  ...
  dialog.querySelector("[data-customer-request-approve]").addEventListener("click", async () => {
    ...
    try {
      ...
      dialog.close(); renderCurrentArea();
      if (validated) printCustomerRequestSheet(validated);
    } catch (error) { alert(error.message || "No fue posible validar y firmar la solicitud."); }
  });
...
function openCustomerRequestDialog(request = null, review = false, returnToList = false, viewOnly = false) {
  const dialog = ensureCustomerRequestDialog();
  dialog.returnToRequestList = returnToList;
  const current = { ...customerRequestWithCurrentCustomerData(request || {}) };
  ...
  dialog.querySelector("[data-customer-request-status]").innerHTML = `<i></i> ${escapeHtml(current.status || "Borrador sin enviar")}${current.assignedClientNumber ? ` · ID ${escapeHtml(current.assignedClientNumber)}` : ""}${(isDraft || isPending) ? (isSigned ? " · Firma electrónica registrada" : " · Firma pendiente") : ""}`;
  dialog.querySelector("[data-customer-request-submit]").classList.toggle("hidden", review || viewOnly || !canSubmitCustomerRequest());
  const draftButton = dialog.querySelector("[data-customer-request-draft]");
  draftButton.classList.toggle("hidden", review || viewOnly || !isDraft);
  draftButton.textContent = current.id ? "Guardar cambios" : "Guardar borrador";
  dialog.querySelector("[data-customer-request-review-save]").classList.toggle("hidden", viewOnly || !review || !isPending || !canValidate);
  dialog.querySelector("[data-customer-request-review-save]").textContent = "Guardar temporalmente";
  dialog.querySelector("[data-customer-request-approved-edit]").classList.toggle("hidden", viewOnly || !isValidated || !current.approvedCustomerId);
  dialog.querySelector("[data-customer-request-approved-save]").classList.add("hidden");
  dialog.querySelector("[data-customer-request-sign]").classList.toggle("hidden", !current.id || !(isDraft || isPending) || isSigned || !isOdalizValenciaUser());
  dialog.querySelector("[data-customer-request-sign]").textContent = "Autorizar firma electrónica";
  dialog.querySelector("[data-customer-request-print]").classList.toggle("hidden", !current.id);
  dialog.querySelector("[data-customer-request-print]").textContent = isValidated ? "Imprimir ficha aprobada" : (isSigned ? "Imprimir solicitud firmada" : "Imprimir solicitud");
  dialog.querySelector("[data-customer-request-approve]").classList.toggle("hidden", viewOnly || !review || !isPending || !canValidate);
  dialog.querySelector("[data-customer-request-reject]").classList.toggle("hidden", viewOnly || !review || !isPending || !canValidate);
  dialog.showModal();
}
...
function openCustomerRequestListDialog() {
  ...
  const renderRows = (query = "") => {
    ...
    rows.querySelectorAll("[data-customer-request-list-view]").forEach((button) => button.addEventListener("click", () => {
      const request = requests.find((item) => String(item.id) === String(button.dataset.customerRequestListView));
      if (!request) return;
      dialog.close();
      openCustomerRequestDialog(request, false, true, true);
    }));
    ...
    rows.querySelectorAll("[data-customer-request-list-sign]").forEach((button) => button.addEventListener("click", async () => {
      ...
      if (!request || !confirm(`¿Autorizar la firma electrónica de ${request.requestNumber || "esta solicitud"} como Odaliz Valencia?`)) return;
      try {
        await crmApi(`/customer-requests/${encodeURIComponent(request.id)}/sign`, { method: "POST", body: "{}" });
        openCustomerRequestListDialog();
        alert("Solicitud firmada electrónicamente. Ya puede aprobarse para crear el cliente.");
...
function masterCustomerQuotationData(customer, quotation = {}) {
  return {
    ...
    customerCode: customer.customerCode || customer.code || customer.clientNumber || "",
    clientNumber: customer.clientNumber || customer.customerCode || customer.code || "",
...
function printOpportunityManagementReport(options = {}) {
  ...
  const generatedAt = new Intl.DateTimeFormat("es-SV", { dateStyle: "long", timeStyle: "short", timeZone: "America/El_Salvador" }).format(new Date());
  let reportIndex = 0;
  const reportGroups = Object.entries(rows.reduce((groups, row) => { const seller = row.item.seller || "Sin vendedor"; (groups[seller] ||= []).push(row); return groups; }, {}))
    .sort(([sellerA], [sellerB]) => sellerA.localeCompare(sellerB, "es"));
  const reportRows = reportGroups.map(([seller, sellerRows]) => {
    const sellerAmount = sellerRows.reduce((sum, row) => sum + Number(row.item.amount || 0), 0);
    const orderedRows = [...sellerRows].sort((a, b) => String(a.item.date || "").localeCompare(String(b.item.date || "")) || String(a.item.company || "").localeCompare(String(b.item.company || ""), "es"));
    const detailRows = orderedRows.map(({ item, result }) => {
      reportIndex += 1;
      ...
      return `<tr><td>${reportIndex}</td><td>${escapeHtml(formatDate(result?.date || item.date))}</td><td><strong>${escapeHtml(item.company || "Sin empresa")}</strong><small>${escapeHtml(item.segment || "Sin producto")}</small></td><td>${escapeHtml(seller)}</td><td>${escapeHtml(item.stage || "Sin etapa")}</td><td>${escapeHtml(probabilityLabel(item.probability))}</td><td>${escapeHtml(item.priority || "Sin definir")}</td><td class="money">${formatMoney(item.amount || 0)}</td><td>${escapeHtml(resultLabel)}</td><td><small><b>Contacto:</b> ${escapeHtml(item.contact || "—")} · ${escapeHtml(item.phone || "—")}</small><small><b>Próxima acción:</b> ${escapeHtml(item.nextAction || "—")}</small><small><b>Última gestión:</b> ${escapeHtml([formatDate(latest.date), latest.stage, latest.comment].filter(Boolean).join(" · ") || "—")}</small><small><b>Observación:</b> ${escapeHtml(result?.comment || item.note || "—")}</small></td></tr>`;
    }).join("");
    return `<tr class="seller-heading"><td colspan="10"><strong>${escapeHtml(seller)}</strong><span>${sellerRows.length} oportunidades · ${formatMoney(sellerAmount)}</span></td></tr>${detailRows}<tr class="seller-validation"><td colspan="10"><span>Validado por ${escapeHtml(seller)}</span><i></i><small>Firma</small><i></i><small>Fecha</small></td></tr>`;
  }).join("");
  ...
  <style>.seller-heading td{padding:10px 12px;background:#dcefe9;border-top:2px solid #188b78;color:#126452}.seller-heading td strong{font-size:14px}.seller-heading td span{float:right;font-weight:800}.seller-validation td{height:58px;padding:17px 12px;background:#f7fafb;color:#405169}.seller-validation span{font-weight:800}.seller-validation i{display:inline-block;width:150px;margin:0 8px;border-bottom:1px solid #405169}.seller-validation small{display:inline;min-width:0}</style><header class="head"><div><div class="brand">KONFI</div><h1>Reporte gerencial de oportunidades</h1><p>${isClosed ? "Oportunidades cerradas" : options.status === "active" ? "Pipeline de oportunidades en venta" : "Reporte integral de oportunidades"}</p></div><div><small>Periodo: ${escapeHtml(state.period)}</small><small>Generado: ${escapeHtml(generatedAt)}</small><small>Usuario: ${escapeHtml(state.currentUser?.name || roleDisplayName())}</small></div></header>
...
function renderBankAvailabilityStatistics() {
  ...
  const updateReportIcon = `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="4" width="14" height="16" rx="2"/><path d="M8 8h8M8 12h3M13 12h3M8 16h3M13 16h3"/></svg>`;
  const bankReportIcon = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m3 9 9-5 9 5M5 10h14M6 10v7M10 10v7M14 10v7M18 10v7M4 20h16"/></svg>`;
  const availabilityReportIcon = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l3 3v15H6zM15 3v4h4M9 17l2-3 2 1 3-4"/></svg>`;
  const archiveRows = [...history].reverse().map((item) => { const accounts = Object.values(item.balances || {}); const netAvailability = Number(item.reportData?.netAvailability || 0); return `<tr><td><strong>${escapeHtml(formatDate(item.date))}</strong><small>${item.completedAt ? `Archivada ${escapeHtml(new Date(item.completedAt).toLocaleString("es-SV"))}` : "Corte archivado"}</small></td><td>${accounts.length} ${accounts.length === 1 ? "cuenta" : "cuentas"}</td><td class="money">${formatMoney(item.total)}</td><td class="money net-availability ${netAvailability < 0 ? "negative" : ""}">${formatMoney(netAvailability)}</td><td><span class="availability-archive-actions"><button type="button" data-bank-update-archive-print="${escapeHtml(item.id)}" title="Reporte de actualización de saldos" aria-label="Imprimir reporte de actualización de saldos">${updateReportIcon}</button><button type="button" data-bank-archive-print="${escapeHtml(item.id)}" title="Reporte detallado de bancos" aria-label="Imprimir reporte detallado de bancos">${bankReportIcon}</button><button type="button" data-availability-archive-print="${escapeHtml(item.id)}" title="Reporte de disponibilidad neta" aria-label="Imprimir reporte de disponibilidad neta">${availabilityReportIcon}</button></span></td></tr>`; }).join("");
  return `<section class="bank-availability-module"><div class="availability-statistics-shell"><header><div><span>Historial financiero</span><h2>Estadística de disponibilidad</h2><p>Evolución de los saldos bancarios archivados durante los últimos 15 días.</p></div><strong>${history.length} ${history.length === 1 ? "corte" : "cortes"}</strong></header><article class="availability-history-chart"><div class="availability-statistics-title"><div><h3>Saldos de bancos</h3><p>Historial móvil de 15 días</p></div>${history.length ? `<strong>${formatMoney(history.at(-1).total)}<small>Último saldo archivado</small></strong>` : ""}</div>${history.length ? `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Gráfico lineal de saldos bancarios archivados"><g class="chart-grid">${grid}</g><polyline class="chart-area-line" points="${points}"/><g class="chart-points">${dots}</g><g class="chart-labels">${labels}</g></svg>${history.length === 1 ? `<p class="availability-chart-note">El gráfico comenzará a mostrar la tendencia cuando se archive el siguiente corte.</p>` : ""}` : `<div class="availability-statistics-empty"><strong>Aún no hay disponibilidades archivadas</strong><span>El primer corte aparecerá aquí al usar “Archivar disponibilidad” en Datos.</span></div>`}</article><section class="availability-archive-list"><header><div><h3>Disponibilidades archivadas</h3><p>Consulta e imprime cualquier corte conservado dentro del historial.</p></div></header>${history.length ? `<div><table><thead><tr><th>Fecha del corte</th><th>Detalle</th><th>Total bancario</th><th>Disponibilidad neta</th><th>Documento</th></tr></thead><tbody>${archiveRows}</tbody></table></div>` : `<p class="availability-statistics-empty">No hay documentos disponibles para imprimir.</p>`}</section></div></section>`;
}
...
function printArchivedDerivedAvailability(archiveId) {
  const archive = (state.bankAvailabilityHistory || []).find((item) => item.id === archiveId);
  if (!archive) { alert("No se encontró la disponibilidad archivada."); return; }
  const storedData = archive.reportData || {}, signatures = archive.signatures || {};
  const data = { ...storedData };
  data.bankBalance = Number(data.bankTotal || 0) + Number(data.pendingDeposits || 0) - Number(data.checksTotal || 0);
  data.operatingBalance = data.bankBalance - Number(data.inventory || 0) - Number(data.reserves || 0);
  data.grossAvailability = data.operatingBalance - Number(data.commissions || 0) - Number(data.laborReserve || 0) - Number(data.fiscalReserve || 0);
  data.netAvailability = data.grossAvailability - Number(data.otherExpenses || 0);
  const logoUrl = new URL("assets/konfi-logo.png", window.location.href).href;
  const rows = [["Saldo bancario contable",data.bankTotal,""],["Remesas pendientes de reflejar en banco",data.pendingDeposits,"(+)"] ,["Cheques pendientes de cobro",data.checksTotal,"(−)"],["Saldo de bancos",data.bankBalance,"","subtotal"],["Cuenta de inventario",data.inventory,"(−)"],["Cuentas de reserva",data.reserves,"(−)"],["Saldo operativo",data.operatingBalance,"","subtotal"],["Comisiones",data.commissions,"(−)"],["Reserva laboral",data.laborReserve,"(−)"],["Reserva fiscal",data.fiscalReserve,"(−)"],["Disponibilidad bruta",data.grossAvailability,"","highlight"],["Otros gastos",data.otherExpenses,"(−)"],["Disponibilidad neta",data.netAvailability,"","total"]];
  const signatureDefinitions = [{key:"prepared",name:"Guadalupe Herrera",role:"Elaborado por"},{key:"reviewed",name:"Claudia Merino",role:"Revisado por"},{key:"authorized",name:"Luis Valladares",role:"Autorizado por"}];
  const popup = window.open("", "_blank", "width=900,height=1000");
  if (!popup) { alert("Permite las ventanas emergentes para imprimir la disponibilidad archivada."); return; }
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte de disponibilidad archivada</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{margin:0;background:#e9eef4;color:#173552;font:12px Arial,sans-serif}.page{width:190mm;min-height:267mm;margin:18px auto;padding:15mm;background:#fff}.head{display:flex;align-items:center;justify-content:space-between;padding-bottom:15px;border-bottom:2px solid #188b78}.head img{width:145px}.head div{text-align:right}.head h1{margin:0 0 5px;font-size:24px}.head p{margin:2px 0;color:#667b8f}.summary{display:flex;justify-content:space-between;align-items:center;margin:18px 0 14px;padding:14px 16px;border:1px solid #bfd6df;border-left:4px solid #188b78;background:#f4f8fa}.summary strong{color:#087763;font-size:24px}.summary.negative{border-color:#e2bcc2;border-left-color:#a33b4b;background:#fff5f6}.summary.negative strong{color:#9f2f40}table{width:100%;border-collapse:collapse}th,td{padding:8px 11px;border:1px solid #c8d7e2}th{text-align:left;background:#f5f8fb}.sign{width:54px;text-align:center;color:#64748b;font-weight:800}.money{width:145px;text-align:right;font-weight:800}.subtotal th,.subtotal td{background:#dce9f3}.highlight th,.highlight td{background:#d8f1eb;color:#08735f}.total th,.total td{background:#176c62;color:#fff;font-size:15px}.total.negative th,.total.negative td{background:#8f3445}.formula{padding:9px 11px;border-left:3px solid #45b99f;background:#f0f6f6;color:#52647a;font-size:10px}.signatures{display:grid;grid-template-columns:repeat(3,1fr);gap:22px;margin-top:55px;text-align:center}.signature{padding-top:8px;border-top:1px solid #2b6f73}.signature strong,.signature span,.signature small{display:block}.signature span{margin:4px 0;color:#178371;font-size:9px;font-weight:800;text-transform:uppercase}.signature small{color:#64748b;font-size:8px}.actions{position:fixed;right:20px;bottom:20px}.actions button{padding:11px 16px;border:0;border-radius:8px;background:#167b68;color:#fff;font-weight:800}@media print{*{-webkit-print-color-adjust:exact;print-color-adjust:exact}body{background:#fff}.page{width:auto;min-height:0;margin:0;padding:0}.actions{display:none}}</style></head><body><main class="page"><header class="head"><img src="${escapeHtml(logoUrl)}" alt="KONFI"><div><h1>Reporte de disponibilidad</h1><p>Fecha del corte: ${escapeHtml(formatDate(archive.date))}</p><p>Disponibilidad archivada</p></div></header><section class="summary ${Number(data.netAvailability || 0) < 0 ? "negative" : ""}"><span>Disponibilidad neta</span><strong>${formatMoney(data.netAvailability || 0)}</strong></section><table><tbody>${rows.map(([label,value,sign,className=""]) => `<tr class="${className} ${className === "total" && Number(value || 0) < 0 ? "negative" : ""}"><th>${escapeHtml(label)}</th><td class="sign">${sign}</td><td class="money">${formatMoney(sign ? Math.abs(Number(value || 0)) : Number(value || 0))}</td></tr>`).join("")}</tbody></table><p class="formula">Disponibilidad neta = saldo de bancos − inventario − reservas − comisiones − reserva laboral − reserva fiscal − otros gastos.</p><section class="signatures">${signatureDefinitions.map((definition) => { const signature = signatures[definition.key]; return `<div class="signature"><strong>${escapeHtml(signature?.signerName || definition.name)}</strong><span>${escapeHtml(signature?.signerRole || definition.role)}</span><small>${signature?.signed ? `Firma electrónica ${escapeHtml(signature.signatureCode || "")}<br>${escapeHtml(new Date(signature.signedAt).toLocaleString("es-SV"))}` : "Firma no registrada en este corte"}</small></div>`; }).join("")}</section></main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button></nav></body></html>`);
  popup.document.close();
}
...
function printBankAvailabilityVerticalReport() {
  ...
  const checksTotal = state.pendingChecks.reduce((sum, item) => sum + Number(item.amount || 0), 0);
  const groups = pendingExpenseGroups();
  const namedTotal = (name) => groups.filter((group) => normalizeKey(group.name) === normalizeKey(name)).reduce((sum, group) => sum + Number(group.total || 0), 0);
  const reservedNames = new Set(["comisiones", "reserva laboral", "reserva fiscal"]);
  const pendingDeposits = Number(state.bankPendingDeposits || 0);
  const inventory = balanceFor("bank-hipotecario");
  const reserves = balanceFor("bank-azul-laboral", "bank-azul-fiscal", "bank-bac-ahorro");
  const bankBalance = total + pendingDeposits - checksTotal;
  const operatingBalance = bankBalance - inventory - reserves;
  const commissions = namedTotal("comisiones");
  const laborReserve = namedTotal("reserva laboral");
  const fiscalReserve = namedTotal("reserva fiscal");
  const grossAvailability = operatingBalance - commissions - laborReserve - fiscalReserve;
  const otherExpenses = groups.filter((group) => !reservedNames.has(normalizeKey(group.name))).reduce((sum, group) => sum + Number(group.total || 0), 0);
  const netAvailability = grossAvailability - otherExpenses;
  const logoUrl = new URL("assets/konfi-logo.png", window.location.href).href;
  const rows = [["Saldo bancario contable",total,""],["Remesas pendientes de reflejar en banco",pendingDeposits,"(+)"] ,["Cheques pendientes de cobro",checksTotal,"(−)"],["Saldo de bancos",bankBalance,"","subtotal"],["Cuenta de inventario",inventory,"(−)"],["Cuentas de reserva",reserves,"(−)"],["Saldo operativo",operatingBalance,"","subtotal"],["Comisiones",commissions,"(−)"],["Reserva laboral",laborReserve,"(−)"],["Reserva fiscal",fiscalReserve,"(−)"],["Disponibilidad bruta",grossAvailability,"","highlight"],["Otros gastos",otherExpenses,"(−)"],["Disponibilidad neta",netAvailability,"","total"]];
  const popup = window.open("", "_blank", "width=900,height=1000");
  if (!popup) { alert("Permite las ventanas emergentes para imprimir la disponibilidad."); return; }
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Disponibilidad financiera</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}body{margin:0;background:#e9eef4;color:#17233a;font:12px Arial,sans-serif}.page{width:190mm;min-height:267mm;margin:18px auto;padding:15mm;background:#fff}.head{display:flex;align-items:center;justify-content:space-between;padding-bottom:12px;border-bottom:3px solid #248f7d}.head img{width:128px;height:auto}.head div{text-align:right}.head h1{margin:0 0 5px;font-size:24px}.head p{margin:0;color:#64748b}.summary{margin:18px 0 14px;padding:14px 17px;border-radius:10px;background:#173b62;color:#fff;display:flex;align-items:center;justify-content:space-between}.summary span{font-weight:700}.summary strong{font-size:25px;color:#6ee0c8}.summary strong.negative{color:#ff9b9b}table{width:100%;border-collapse:collapse}th,td{padding:9px 11px;border:1px solid #cad5e2}th{text-align:left;background:#f7f9fc}td.sign{width:54px;text-align:center;color:#64748b;font-weight:800}td.amount{width:145px;text-align:right;font-weight:700}.subtotal th,.subtotal td{background:#e9f0f8;font-weight:800}.highlight th,.highlight td{background:#dff4ef;color:#08745f;font-weight:800}.total th,.total td{background:#176c62;color:#fff;font-size:15px;font-weight:900}.negative{color:#b42318!important}.formula{margin:12px 0;color:#52647a;font-size:10px}.signatures{display:grid;grid-template-columns:repeat(3,1fr);gap:22px;margin-top:62px;text-align:center}.signature{padding-top:8px;border-top:1px solid #27364a}.signature strong{display:block;margin-bottom:4px}.signature span{color:#64748b;font-size:10px}.actions{position:fixed;right:20px;bottom:20px}.actions button{padding:11px 16px;border:0;border-radius:8px;background:#167b68;color:#fff;font-weight:800}@media print{body{background:#fff}.page{width:auto;min-height:0;margin:0;padding:0}.actions{display:none}}</style></head><body><main class="page"><header class="head"><img src="${escapeHtml(logoUrl)}" alt="KONFI"><div><h1>Reporte de disponibilidad</h1><p>Fecha de emisión: ${escapeHtml(new Date().toLocaleString("es-SV"))}</p></div></header><section class="summary"><span>Disponibilidad neta</span><strong class="${netAvailability < 0 ? "negative" : ""}">${formatMoney(netAvailability)}</strong></section><table><tbody>${rows.map(([label,value,sign,className=""]) => `<tr class="${className}"><th>${escapeHtml(label)}</th><td class="sign">${sign}</td><td class="amount ${!sign && value < 0 ? "negative" : ""}">${formatMoney(sign ? Math.abs(value) : value)}</td></tr>`).join("")}</tbody></table><p class="formula">Disponibilidad neta = saldo de bancos − inventario − reservas − comisiones − reserva laboral − reserva fiscal − otros gastos.</p><section class="signatures"><div class="signature"><strong>Guadalupe Herrera</strong><span>Elaborado por</span></div><div class="signature"><strong>Claudia Merino</strong><span>Revisado por</span></div><div class="signature"><strong>Luis Valladares</strong><span>Autorizado por</span></div></section></main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button></nav></body></html>`);
  popup.document.head?.insertAdjacentHTML("beforeend", `<style>
    body{background:linear-gradient(135deg,#e7edf5,#dce8e7);color:#152842}
    .page{border:1px solid #d6e1e8;border-radius:3px;box-shadow:0 14px 38px rgba(15,39,65,.12)}
    .head{margin:0 0 17px;padding:0 0 14px;background:#fff;border-bottom:2px solid #188b78;color:#173552}
    .head img{width:148px;padding:0;border:0;border-radius:0;background:transparent;box-shadow:none}
    .head h1{color:#173552;font-size:25px;letter-spacing:-.02em}.head p{color:#667b8f}
    .summary{margin:0 0 14px;padding:12px 15px;border:1px solid #bfd6df;border-left:4px solid #188b78;background:#f4f8fa;color:#173552;box-shadow:none}
    .summary span{letter-spacing:.025em}.summary strong{color:#087763}
    .summary:has(.negative){border-color:#e2bcc2;border-left-color:#a33b4b;background:#fff5f6}.summary:has(.negative) strong{color:#9f2f40!important}
    table{overflow:hidden;border:1px solid #b9ccda;border-radius:8px}
    th,td{border-color:#c8d7e2;padding:9px 12px}th{background:#f5f8fb;color:#19324f}
    tbody tr:nth-child(even):not(.subtotal):not(.highlight):not(.total) th,tbody tr:nth-child(even):not(.subtotal):not(.highlight):not(.total) td{background:#edf3f7}
    td.sign{background:#edf3f7;color:#50708c}td.amount{color:#173552}
    .subtotal th,.subtotal td{background:#dce9f3;color:#173f66;border-color:#b7ccdc}
    .highlight th,.highlight td{background:#d8f1eb;color:#08735f;border-color:#a9d9ce}
    .total th,.total td{background:linear-gradient(90deg,#176b62,#168374);color:#fff;border-color:#176b62}
    .total:has(.negative) th,.total:has(.negative) td{background:linear-gradient(90deg,#713243,#963846);border-color:#713243}.total .negative{color:#fff!important}
    .formula{padding:9px 11px;border-left:3px solid #45b99f;background:#f0f6f6;color:#476275}
    .signatures{gap:28px}.signature{border-color:#2b6f73}.signature strong{color:#173b5d}.signature span{color:#178371;font-weight:700;text-transform:uppercase;letter-spacing:.06em}
    .actions button{background:linear-gradient(90deg,#176b62,#159078);box-shadow:0 8px 22px rgba(14,78,70,.25)}
    @media print{*{-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{border:0;box-shadow:none}.head{margin:0 0 17px}}
  </style>`);
...
function renderBankAvailability() {
  ...
  const daily = state.bankAvailability.dailyAvailability || {};
  const latestDaily = daily.latest;
  const progress = daily.progress || { updated: 0, total: accounts.length };
  const dailyStatus = Number(progress.updated || 0) > 0
    ? `<span class="bank-daily-status"><b>Nuevo corte en preparación</b> ${Number(progress.updated)} de ${Number(progress.total || accounts.length)} bancos actualizados</span>`
    : latestDaily
...
async function printBankAvailabilityReport(archivedReport = null, archivedDate = "") {
  ...
  const detailDialogs = accounts.flatMap((account) => ["charges", "credits"].map((type) => {
    ...
    const totalKey = type === "charges" ? "charges" : "credits";
    const movements = (account.movements || []).filter((movement) => Number(movement[amountKey] || 0) > 0);
    if (!movements.length) return "";
    const movementRows = movements.map((movement, index) => `<tr><td>${index + 1}</td><td>${movement.date ? formatDate(movement.date) : "—"}</td><td>${escapeHtml(movement.description || "Movimiento bancario")}</td><td>${escapeHtml(movement.reference || "—")}</td><td class="${type === "charges" ? "charge" : "credit"}">${formatMoney(movement[amountKey] || 0)}</td><td class="money">${formatMoney(movement.balance || 0)}</td></tr>`).join("");
    return `<dialog class="bank-detail-dialog" id="bank-detail-${escapeHtml(account.id)}-${type}"><div class="detail-dialog-head"><div><small>${type === "charges" ? "DETALLE DE CARGOS" : "DETALLE DE ABONOS"}</small><h2>${escapeHtml(account.bank)}</h2><span>${escapeHtml(account.account)} · ${account.date ? formatDate(account.date) : "Sin fecha"}</span></div><button type="button" onclick="this.closest('dialog').close()">×</button></div><table class="detail-table"><thead><tr><th>#</th><th>Fecha</th><th>Descripción</th><th>Referencia</th><th>${type === "charges" ? "Cargo" : "Abono"}</th><th>Saldo resultante</th></tr></thead><tbody>${movementRows}</tbody><tfoot><tr><th colspan="4">Total</th><th>${formatMoney(account[totalKey] || 0)}</th><th></th></tr></tfoot></table><div class="detail-dialog-actions"><button type="button" onclick="this.closest('dialog').close()">Cerrar detalle</button></div></dialog>`;
  })).join("");
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte de disponibilidad bancaria</title><style>
    @page{size:landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17233a;font:12px Arial,sans-serif;background:#eef2f7}.sheet{width:100%;max-width:1400px;margin:20px auto;padding:28px;background:#fff}header{display:flex;justify-content:space-between;align-items:end;border-bottom:3px solid #28a98c;padding-bottom:14px;margin-bottom:22px}h1{margin:3px 0;font-size:27px}header p{margin:0;color:#64748b}.status{color:#147d69;font-weight:700}.matrix,.detail-table{width:100%;border-collapse:collapse;table-layout:fixed}.matrix th,.matrix td,.detail-table th,.detail-table td{border:1px solid #cbd5e1;padding:10px 8px;text-align:right}.matrix thead th,.detail-table thead th{background:#172f51;color:#fff;text-align:center}.matrix thead th:first-child{width:170px}.matrix thead strong,.matrix thead small{display:block}.matrix thead small{margin-top:4px;color:#cbd5e1;font-size:9px}.matrix tbody th{text-align:left;background:#f8fafc}.amount-cell{display:flex;align-items:center;justify-content:flex-end;gap:7px}.expand-btn{flex:0 0 22px;width:22px;height:22px;padding:0;border:1px solid #8095ad;border-radius:50%;background:#fff;color:#173b62;font-weight:900;cursor:pointer}.expand-btn:hover{border-color:#28a98c;color:#087f5b}.money{font-weight:700}.credit{color:#087f5b}.charge{color:#b42318}.matrix tfoot th,.matrix tfoot td{background:#e8f5f1;font-size:14px;font-weight:800}.formula{margin:16px 0 0;color:#475569}.bank-detail-dialog{width:min(920px,92vw);padding:0;border:0;border-radius:14px;box-shadow:0 24px 70px rgba(15,35,60,.28)}.bank-detail-dialog::backdrop{background:rgba(15,35,60,.48)}.detail-dialog-head{display:flex;justify-content:space-between;align-items:center;padding:17px 20px;background:#172f51;color:#fff}.detail-dialog-head small{color:#6ee0c8;font-weight:800}.detail-dialog-head h2{margin:3px 0;font-size:21px}.detail-dialog-head span{color:#cbd5e1}.detail-dialog-head button{width:34px;height:34px;border:1px solid #69809c;border-radius:9px;background:#234466;color:#fff;font-size:22px}.detail-table{font-size:11px}.detail-table th:nth-child(1){width:5%}.detail-table th:nth-child(2){width:12%}.detail-table th:nth-child(3){width:35%}.detail-table th:nth-child(4){width:18%}.detail-table th:nth-child(n+5){width:15%}.detail-table td:nth-child(-n+4){text-align:left}.detail-table tfoot th,.detail-table tfoot td{background:#edf5f3;font-weight:800}.detail-dialog-actions{display:flex;justify-content:flex-end;padding:13px 18px}.detail-dialog-actions button{padding:9px 14px;border:0;border-radius:8px;background:#173b62;color:#fff;font-weight:700}.actions{position:fixed;right:18px;bottom:18px;display:flex;gap:8px}.actions button{padding:11px 15px;border:0;border-radius:8px;background:#173b62;color:#fff;font-weight:700}.actions button:first-child{background:#16866d}@media print{body{background:#fff}.sheet{margin:0;padding:0;max-width:none}.actions,.expand-btn,.bank-detail-dialog{display:none}}
  </style></head><body><main class="sheet"><header><div><p>FINANCIERA · DISPONIBILIDAD BANCARIA</p><h1>Reporte de actualización de saldos</h1><p>${archivedDate ? `Corte archivado ${escapeHtml(formatDate(archivedDate))}` : `Generado ${escapeHtml(new Date().toLocaleString("es-SV"))}`}</p></div><div class="status">${escapeHtml(status)}</div></header><table class="matrix"><thead><tr><th>Concepto</th>${headings}<th>Total consolidado</th></tr></thead><tbody>${row("Fecha de actualización", "date")} ${row("Saldo anterior", "previousBalance", "money")} ${row("(−) Cargos", "charges", "money charge", true)}${row("(+) Abonos", "credits", "money credit", true)}</tbody><tfoot><tr><th>Saldo nuevo</th>${accounts.map((item) => `<td>${formatMoney(item.newBalance || 0)}</td>`).join("")}<td>${formatMoney(report.newTotal || 0)}</td></tr></tfoot></table><p class="formula"><b>Comprobación:</b> saldo anterior − cargos + abonos = saldo nuevo. Totales del corte: ${formatMoney(report.previousTotal || 0)} − ${formatMoney(report.chargesTotal || 0)} + ${formatMoney(report.creditsTotal || 0)} = <b>${formatMoney(report.newTotal || 0)}</b>.</p></main>${detailDialogs}<nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button><button onclick="window.close()">Cerrar</button></nav><script>document.querySelectorAll('[data-open-bank-detail]').forEach(function(button){button.addEventListener('click',function(){document.getElementById('bank-detail-'+button.dataset.openBankDetail).showModal();});});<\/script></body></html>`);
  popup.document.close();
}
...
const pendingExpenseCostCenters = [
  "Servicios", "Planilla", "Proveedores", "Caja Chica", "Inventario",
  "Planilla Previsional", "Impuestos", "Reserva Laboral", "Reserva Fiscal",
  "Comisiones", "Descuentos FSV", "Bodega Arte y Color", "Honorarios"
];
...
function bankMovementCell(field, value) {
  if (bankFieldType(field) !== "number") return escapeHtml(value ?? "");
...
async function printBankProvisionReport(account) {
  const items = await apiJson(`/api/bank-availability/${encodeURIComponent(account.id)}/provisions`);
  if (!items.length) return alert("Esta cuenta todavía no tiene provisiones registradas.");
  const popup = window.open("", "_blank", "width=1180,height=850");
  if (!popup) return alert("Permite las ventanas emergentes para imprimir el reporte.");
  const sums = items.reduce((total, item) => ({ gross:total.gross + Number(item.gross || 0), net:total.net + Number(item.net || 0), tax:total.tax + Number(item.taxProvision || 0), labor:total.labor + Number(item.labor || 0), commission:total.commission + Number(item.commission || 0) }), { gross:0, net:0, tax:0, labor:0, commission:0 });
  const rows = items.map((item) => `<tr><td>${escapeHtml(formatDate(item.date))}</td><td>${escapeHtml(item.customerName || "—")}</td><td>${escapeHtml(item.paymentType || "—")}</td><td>${escapeHtml(item.seller || "—")}</td><td>${formatMoney(item.gross)}</td><td>${formatMoney(item.net)}</td><td>${formatMoney(item.taxProvision)}</td><td>${formatMoney(item.labor)}</td><td>${formatMoney(item.commission)}</td><td>${formatMoney(Number(item.taxProvision || 0) + Number(item.labor || 0) + Number(item.commission || 0))}</td></tr>`).join("");
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte de provisiones</title><style>@page{size:landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17233a;font:11px Arial,sans-serif}.sheet{padding:22px}header{display:flex;justify-content:space-between;align-items:end;border-bottom:3px solid #28a98c;margin-bottom:18px;padding-bottom:12px}h1{margin:3px 0;font-size:25px}header p{margin:2px 0;color:#64748b}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:7px;text-align:right}th{background:#172f51;color:#fff}th:nth-child(-n+4),td:nth-child(-n+4){text-align:left}tfoot td{background:#e8f5f1;font-weight:800}.actions{position:fixed;right:18px;bottom:18px;display:flex;gap:8px}.actions button{padding:11px 15px;border:0;border-radius:8px;background:#173b62;color:#fff;font-weight:700}.actions button:first-child{background:#16866d}@media print{.sheet{padding:0}.actions{display:none}}</style></head><body><main class="sheet"><header><div><p>FINANCIERA · PROVISIONES BANCARIAS</p><h1>Reporte de provisiones</h1><p>${escapeHtml(account.bank)} · ${escapeHtml(account.account)}</p></div><p>Generado ${escapeHtml(new Date().toLocaleString("es-SV"))}</p></header><table><thead><tr><th>Fecha</th><th>Cliente</th><th>Concepto</th><th>Vendedor</th><th>Depositado</th><th>Neto</th><th>Impuestos 13%</th><th>Laboral 7%</th><th>Comisión</th><th>Total provisiones</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="4">TOTAL</td><td>${formatMoney(sums.gross)}</td><td>${formatMoney(sums.net)}</td><td>${formatMoney(sums.tax)}</td><td>${formatMoney(sums.labor)}</td><td>${formatMoney(sums.commission)}</td><td>${formatMoney(sums.tax + sums.labor + sums.commission)}</td></tr></tfoot></table></main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button><button onclick="window.close()">Cerrar</button></nav></body></html>`);
  popup.document.close();
}
...
function renderFinancialIncome() {
  ...
  const chart = monthly.map((month) => `<article class="financial-income-month">
    <div class="financial-income-bars"><span class="gross" style="height:${Math.min(100, (month.gross / monthlyGoal) * 100)}%" title="Efectivo ${formatMoney(month.gross)}"></span><span class="labor" style="height:${Math.min(100, (month.labor / monthlyGoal) * 100)}%" title="Provisión ${formatMoney(month.labor)}"></span></div>
    <strong>${escapeHtml(month.short)}</strong><div class="financial-income-month-values"><span><i class="gross"></i><b>Efectivo</b><em>${formatMoney(month.gross)}</em></span><span><i class="labor"></i><b>Provisión</b><em>${formatMoney(month.labor)}</em></span></div><small>${month.count} ${month.count === 1 ? "remesa" : "remesas"}</small>
  </article>`).join("");
  ...
  return `<section class="financial-income-module">
    ...
    <section class="financial-income-chart"><header><div><span>Comparativo mensual</span><h3>Septiembre a diciembre</h3></div><div class="financial-income-goal"><small>Meta mensual</small><strong>${formatMoney(monthlyGoal)}</strong></div></header><div class="financial-income-chart-body"><div class="financial-income-scale" aria-hidden="true"><span style="bottom:100%"><b>$250K</b></span><span style="bottom:80%"><b>$200K</b></span><span style="bottom:60%"><b>$150K</b></span><span style="bottom:40%"><b>$100K</b></span><span style="bottom:20%"><b>$50K</b></span><span style="bottom:0"><b>$0</b></span></div>${chart}</div></section>
...
async function openBankMaintenance(accountId) {
  ...
  const selectedProvisionIds = new Set();
  ...
  const render = (formOpen = false, editing = null, bulkOpen = false) => {
    ...
    dialog.innerHTML = `<div class="bank-maintenance-shell"><header><div><span>Movimientos bancarios</span><h2>${escapeHtml(account.bank)}</h2><p>${escapeHtml(account.account)} · ${records.length} movimientos</p></div><button type="button" data-bank-close aria-label="Cerrar">×</button></header>
      <section class="bank-maintenance-summary"><div><small>Último saldo</small><strong>${formatMoney((records[0] || account.latest)?.balance || 0)}</strong><span>${records[0] ? formatDate(records[0].date) : "Sin movimientos"}</span></div><aside><button type="button" data-bank-provisions-report>▤ Imprimir provisiones</button><button type="button" class="bank-provision-button" data-bank-provision disabled>Provisionar <b data-bank-provision-count>0</b></button><button type="button" data-bank-paste>▦ Pegar bloque</button><button type="button" data-bank-new>＋ Agregar línea</button></aside></section>
    ...
    dialog.querySelector("[data-bank-provisions-report]").onclick = () => printBankProvisionReport(account).catch((error) => alert(error.message || "No se pudo imprimir el reporte de provisiones."));
    const refreshProvisionSelection = () => {
      const button = dialog.querySelector("[data-bank-provision]");
      const count = selectedProvisionIds.size;
    ...
    dialog.querySelector("[data-bank-provision]").addEventListener("click", () => {
      ...
      const net = gross / 1.1475; const vat = net * 0.13; const taxProvision = vat; const labor = net * 0.07;
      const sellerRates = [["Odaliz Valencia",.02],["Amadeo Alfaro",.09],["Marco Velado",.09],["Erick Orantes",.04],["Yanira Merino",.04],["Gabriela Amador",.04],["Marjorie Morales",.04]];
      const provisionDialog = document.createElement("dialog"); provisionDialog.className = "bank-provision-dialog";
      provisionDialog.innerHTML = `<form><header><div><span>Provisiones bancarias</span><h2>Confirmar provisión</h2><p>${chosen.length} ${chosen.length === 1 ? "movimiento seleccionado" : "movimientos seleccionados"}</p></div><button type="button" data-provision-close>×</button></header><section class="bank-provision-details"><label><span>Nombre del cliente</span><input name="customerName" required maxlength="180" autocomplete="off" placeholder="Escribe el nombre del cliente"></label><label><span>Concepto del depósito</span><select name="paymentType" required><option value="">Seleccionar concepto…</option><option>Anticipo</option><option>Abono</option><option>Cancelación de saldo</option></select></label><label><span>Vendedor para comisión</span><select name="seller" required><option value="">Seleccionar vendedor…</option>${sellerRates.map(([seller,rate]) => `<option value="${escapeHtml(seller)}" data-rate="${rate}">${escapeHtml(seller)} · ${rate*100}%</option>`).join("")}</select></label></section><section class="bank-provision-breakdown"><div class="deposit"><span>Valor depositado</span><strong>${formatMoney(gross)}</strong></div><div class="calculation"><span>Operación para obtener el valor neto</span><strong>÷ 1.1475</strong></div><div class="net"><span>Valor neto</span><strong>${formatMoney(net)}</strong></div><div class="tax"><span><b>Provisión de impuestos</b><small>13% del valor neto</small></span><strong>${formatMoney(taxProvision)}</strong></div><div class="labor"><span>Provisión laboral · 7% del neto</span><strong>${formatMoney(labor)}</strong></div><div class="commission"><span data-provision-commission-label>Provisión de comisión del vendedor</span><strong data-provision-commission>—</strong></div><div class="total"><span>Total de provisiones</span><strong data-provision-total>${formatMoney(taxProvision + labor)}</strong></div></section><p class="bank-provision-note">Esta operación registra las provisiones y no altera el saldo bancario del movimiento.</p><footer><button type="button" data-provision-close>Cancelar</button><button type="submit">Confirmar provisión</button></footer></form>`;
...
function openPendingExpenseSettlement(expenseIds, parentDialog) {
  const items = state.pendingExpenses.filter((item) => expenseIds.includes(item.id));
  if (!items.length) return;
  const total = items.reduce((sum,item) => sum + Number(item.amount || 0), 0);
  const accounts = state.bankAvailability.accounts || [];
  const dialog = document.createElement("dialog"); dialog.className = "expense-settlement-dialog";
...
function openPendingExpensesCrud() {
  let query = ""; let editingId = ""; const selectedIds = new Set();
  ...
  const render = () => {
    ...
    const selectedCostCenter = editing?.costCenter || "";
    const costCenterOptions = pendingExpenseCostCenters.map((center) => `<option value="${escapeHtml(center)}" ${center === selectedCostCenter ? "selected" : ""}>${escapeHtml(center)}</option>`).join("");
    dialog.innerHTML = `<section class="pending-expenses-crud"><header><div><span>Mantenimiento financiero</span><h2>Gastos pendientes</h2><p>Selecciona varios gastos para liquidarlos contra una cuenta bancaria.</p></div><button type="button" data-expense-close aria-label="Cerrar">×</button></header><div class="pending-expenses-crud-toolbar"><label><span>⌕</span><input type="search" data-expense-search value="${escapeHtml(query)}" placeholder="Buscar centro, fecha, detalle o monto..."></label><button class="settle" type="button" data-expense-settle ${selectedIds.size ? "" : "disabled"}>Liquidar seleccionados (${selectedIds.size})</button><button type="button" data-expense-new>+ Nuevo registro</button></div><form class="pending-expense-form ${editingId === "new" || editing ? "is-open" : ""}" data-expense-form><input type="hidden" name="id" value="${escapeHtml(editing?.id || "")}"><label><span>Fecha a pagar</span><input name="date" type="date" required value="${escapeHtml(toDateInputValue(editing?.date || ""))}"></label><label><span>Centro de costo</span><select name="costCenter" required><option value="">Seleccionar centro de costo...</option>${costCenterOptions}</select></label><label class="detail"><span>Detalle</span><input name="detail" required value="${escapeHtml(editing?.detail || "")}" placeholder="Descripción del gasto"></label><label><span>Monto</span><input name="amount" type="number" min="0.01" step="0.01" required value="${editing ? Number(editing.amount || 0).toFixed(2) : ""}"></label><div><button type="button" data-expense-cancel>Cancelar</button><button type="submit">${editing ? "Guardar cambios" : "Agregar fila"}</button></div></form><div class="pending-expenses-crud-list"><table><thead><tr><th class="select"></th><th>Fecha</th><th>Centro / costo</th><th>Detalle</th><th>Monto</th><th>Acciones</th></tr></thead><tbody>${rows.map((item) => `<tr class="${selectedIds.has(item.id) ? "is-selected" : ""}"><td class="select"><label class="expense-switch"><input type="checkbox" data-expense-select="${escapeHtml(item.id)}" ${selectedIds.has(item.id) ? "checked" : ""}><i></i></label></td><td>${escapeHtml(item.date)}</td><td><strong>${escapeHtml(item.costCenter)}</strong></td><td>${escapeHtml(item.detail)}</td><td class="money">${formatMoney(item.amount)}</td><td><button type="button" data-expense-edit="${escapeHtml(item.id)}" title="Editar">✎</button><button class="danger" type="button" data-expense-delete="${escapeHtml(item.id)}" title="Eliminar">⌫</button></td></tr>`).join("") || `<tr><td colspan="6" class="empty-state">No se encontraron movimientos.</td></tr>`}</tbody></table></div><footer><span>${selectedIds.size ? `${selectedIds.size} seleccionados` : `${rows.length} registros visibles`}</span><strong>${formatMoney([...selectedIds].reduce((sum,id) => sum + Number(state.pendingExpenses.find((item) => item.id === id)?.amount || 0),0))}</strong></footer></section>`;
    const settleButton = dialog.querySelector("[data-expense-settle]");
    settleButton?.insertAdjacentHTML("beforebegin", `<button type="button" data-expense-delete-selected ${selectedIds.size ? "" : "disabled"} style="border-color:#a9546c;background:#6b3046;color:#ffd5df">Eliminar seleccionados (${selectedIds.size})</button>`);
    settleButton?.remove();
    const expenseHelp = dialog.querySelector(".pending-expenses-crud > header p");
    if (expenseHelp) expenseHelp.textContent = "Selecciona uno o varios gastos para eliminarlos del registro.";
    ...
    dialog.querySelector("[data-expense-delete-selected]")?.addEventListener("click", async () => {
      const ids = [...selectedIds];
      const selected = state.pendingExpenses.filter((item) => ids.includes(item.id));
      const total = selected.reduce((sum, item) => sum + Number(item.amount || 0), 0);
      if (!selected.length || !confirm(`¿Eliminar definitivamente ${selected.length} gastos seleccionados por un total de ${formatMoney(total)}?`)) return;
      const previous = state.pendingExpenses;
      state.pendingExpenses = previous.filter((item) => !selectedIds.has(item.id));
      try {
        await savePendingExpenses();
        selectedIds.clear();
        render();
        renderCommercialSubmenu(areas.financiera);
      } catch (error) {
        state.pendingExpenses = previous;
        alert(error.message || "No se pudieron eliminar los gastos seleccionados.");
        render();
      }
    });
    dialog.querySelectorAll("[data-expense-select]").forEach((input) => input.addEventListener("change", () => { input.checked ? selectedIds.add(input.dataset.expenseSelect) : selectedIds.delete(input.dataset.expenseSelect); render(); }));
...
function wireBankAvailability() {
  ...
  document.querySelector("[data-bank-availability-archive]")?.addEventListener("click", async (event) => {
    if (!confirm("¿Archivar la disponibilidad actual? Los saldos y fechas de hoy serán la apertura del próximo corte.")) return;
    const button = event.currentTarget;
    button.disabled = true;
    try {
      ...
      alert(error.message || "No se pudo archivar la disponibilidad.");
      button.disabled = false;
    }
  });
  ...
  document.querySelectorAll("[data-bank-update-archive-print]").forEach((button) => button.addEventListener("click", () => {
    const archive = (state.bankAvailabilityHistory || []).find((item) => item.id === button.dataset.bankUpdateArchivePrint);
    if (!archive?.updateReport) return alert("No se encontró el reporte de actualización de este corte.");
    printBankAvailabilityReport(archive.updateReport, archive.date).catch((error) => alert(error.message || "No se pudo generar el reporte histórico."));
  }));
...
function normalizeCommercialAgendaTime(value) {
  const compact = String(value || "").trim().toLowerCase().replace(/\s+/g, "").replace(/\./g, "");
  const match = compact.match(/^(\d{1,4})(?::(\d{1,2}))?(am|pm)?$/);
  if (!match) return "";
  let hourText = match[1]; let minuteText = match[2] || ""; const period = match[3] || "";
  if (!match[2] && hourText.length >= 3) { minuteText=hourText.slice(-2); hourText=hourText.slice(0,-2); }
  let hours = Number(hourText); const minutes = Number(minuteText || 0);
  if (minutes > 59 || (period && (hours < 1 || hours > 12)) || (!period && hours > 23)) return "";
  if (period === "am" && hours === 12) hours = 0;
  if (period === "pm" && hours !== 12) hours += 12;
  if (!period && hours >= 1 && hours <= 5) hours += 12;
  if (!period && !((hours >= 7 && hours <= 11) || (hours === 12 && minutes === 0) || (hours >= 13 && hours <= 17))) return "";
...
function renderCommercialAgendaManagement() {
  const selectedDate = state.commercialAgendaDate || todayISO();
  const rows = state.commercialAgenda.flatMap((item) => commercialAgendaItemEvents(item).filter((event) => event.date === selectedDate).map((event) => ({ item, event })));
  const sellers = Object.entries(rows.reduce((grouped, row) => { (grouped[row.item.seller || "Sin vendedor"] ||= []).push(row); return grouped; }, {})).sort(([a], [b]) => a.localeCompare(b, "es"));
  const hours = ["7 a. m.", "8", "9", "10", "11", "12 m.", "1 p. m.", "2", "3", "4", "5 p. m."];
  return `<section class="commercial-agenda-management"><header><div><span>Vista gerencial diaria</span><strong>${escapeHtml(formatDate(selectedDate))}</strong></div><nav><button type="button" data-agenda-date-step="-1" aria-label="Día anterior" title="Día anterior">‹</button><input type="date" data-agenda-management-date value="${escapeHtml(selectedDate)}" aria-label="Fecha del cronograma"><button type="button" data-agenda-date-step="1" aria-label="Día siguiente" title="Día siguiente">›</button></nav></header><div class="commercial-agenda-gantt"><div class="commercial-agenda-gantt-head"><strong>Vendedor</strong><div>${hours.map((hour) => `<span>${hour}</span>`).join("")}</div></div>${sellers.map(([seller, sellerRows]) => { const sorted = sellerRows.sort((a,b) => String(a.event.startTime).localeCompare(String(b.event.startTime))); return `<article class="commercial-agenda-gantt-row" style="--agenda-lanes:${sorted.length}"><header><strong>${escapeHtml(seller)}</strong><span>${sorted.length} ${sorted.length === 1 ? "actividad" : "actividades"}</span></header><div class="commercial-agenda-gantt-track"><i class="commercial-agenda-lunch" aria-hidden="true"></i>${sorted.map(({item,event},index) => { const start = Math.max(420, commercialAgendaMinutes(event.startTime)); const end = Math.min(1020, commercialAgendaMinutes(event.endTime)); const left = (start - 420) / 600 * 100; const width = Math.max((end - start) / 600 * 100, 1.5); return `<button type="button" class="commercial-agenda-gantt-event" data-commercial-agenda-edit="${escapeHtml(item.id)}" style="--event-left:${left}%;--event-width:${width}%;--event-lane:${index}" title="${escapeHtml(`${commercialAgendaTimeLabel(event.startTime)}–${commercialAgendaTimeLabel(event.endTime)} · ${event.prospect || "Sin cliente"} · ${event.activity}`)}"><time>${escapeHtml(commercialAgendaTimeLabel(event.startTime))}–${escapeHtml(commercialAgendaTimeLabel(event.endTime))}</time><strong>${escapeHtml(event.prospect || "Sin cliente")}</strong><span>${escapeHtml(event.activity)}</span></button>`; }).join("")}</div></article>`; }).join("") || `<div class="commercial-agenda-gantt-empty">No hay actividades programadas para este día.</div>`}</div></section>`;
}
...
function renderCommercialAgendaValidation() {
  const rows=commercialAgendaValidationRows(); const effective=rows.filter(({event})=>event.validation?.effective===true).length; const reviewed=rows.filter(({event})=>event.validation?.validatedAt).length; const canValidate=isAdminUser()||isCommercialManagementUser();
  return `<section class="commercial-agenda-validation"><header><div><label><span>Desde</span><input type="date" data-agenda-validation-start value="${escapeHtml(state.commercialAgendaValidationStart)}"></label><label><span>Hasta</span><input type="date" data-agenda-validation-end value="${escapeHtml(state.commercialAgendaValidationEnd)}"></label></div><button type="button" data-agenda-validation-report ${rows.length?"":"disabled"}>▤ Generar reporte</button></header><div class="commercial-agenda-validation-summary"><article><span>Eventos</span><strong>${rows.length}</strong></article><article><span>Validados</span><strong>${reviewed}</strong></article><article><span>Efectivos</span><strong>${effective}</strong></article><article><span>Pendientes</span><strong>${rows.length-reviewed}</strong></article></div><div class="commercial-agenda-validation-table"><table><thead><tr><th>Fecha</th><th>Horario</th><th>Vendedor</th><th>Cliente / prospecto</th><th>Actividad</th><th>Comentario</th><th>Validación gerencial</th></tr></thead><tbody>${rows.map(({item,event})=>{const validation=event.validation||{};const status=validation.validatedAt?(validation.effective?"Efectiva":"No efectiva"):"Pendiente";return `<tr><td>${escapeHtml(formatDate(event.date))}</td><td><strong>${escapeHtml(commercialAgendaTimeLabel(event.startTime))}</strong><small>${escapeHtml(commercialAgendaTimeLabel(event.endTime))}</small></td><td><strong>${escapeHtml(item.seller||"—")}</strong></td><td>${escapeHtml(event.prospect||"—")}</td><td>${escapeHtml(event.activity||"—")}</td><td>${escapeHtml(event.comment||event.result||"Sin comentario")}</td><td><div class="agenda-validation-control"><label class="agenda-validation-switch" title="${canValidate?"Marcar si la actividad fue efectiva":"Solo la Gerencia de Comercialización puede validar"}"><input type="checkbox" data-agenda-validation-event="${escapeHtml(event.id)}" ${validation.effective?"checked":""} ${canValidate?"":"disabled"}><i></i></label><span class="${validation.validatedAt?(validation.effective?"effective":"ineffective"):"pending"}">${status}</span>${validation.validatedAt?`<small>${escapeHtml(validation.validatedBy||"Gerencia")}<br>${escapeHtml(new Date(validation.validatedAt).toLocaleString("es-SV"))}</small>`:""}</div></td></tr>`;}).join("")||`<tr><td colspan="7" class="empty-state">No hay actividades dentro del rango seleccionado.</td></tr>`}</tbody></table></div></section>`;
}
function printCommercialAgendaValidationReport() {
  const rows=commercialAgendaValidationRows(); if(!rows.length)return alert("No hay actividades para generar el reporte.");
  const effective=rows.filter(({event})=>event.validation?.effective===true).length; const reviewed=rows.filter(({event})=>event.validation?.validatedAt).length; const popup=window.open("","_blank","width=1250,height=900"); if(!popup)return alert("El navegador bloqueó la ventana del reporte.");
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Validación de agenda comercial</title><style>@page{size:landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17243c;font:12px Arial,sans-serif}header{display:flex;justify-content:space-between;align-items:end;border-bottom:3px solid #148f78;padding-bottom:12px}h1{margin:3px 0;font-size:27px}header p{margin:0;color:#64748b}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:16px 0}.summary div{padding:12px;border:1px solid #cad5e2;border-radius:9px}.summary span{display:block;color:#64748b;font-size:10px;font-weight:bold;text-transform:uppercase}.summary strong{font-size:22px;color:#087c68}table{width:100%;border-collapse:collapse}th{padding:9px;background:#173b61;color:#fff;text-align:left;font-size:10px}td{padding:8px;border-bottom:1px solid #d8e0e9;vertical-align:top}tr:nth-child(even){background:#f4f7fa}.yes{color:#087c68;font-weight:bold}.no{color:#ba3c37;font-weight:bold}.pending{color:#9a6b13;font-weight:bold}footer{margin-top:15px;color:#64748b;font-size:10px}.actions{position:fixed;right:16px;bottom:16px}.actions button{padding:11px 16px;border:0;border-radius:8px;background:#148f78;color:#fff;font-weight:bold}@media print{.actions{display:none}}</style></head><body><header><div><small>COMERCIALIZACIÓN · AGENDA</small><h1>Validación de actividades</h1><p>${escapeHtml(formatDate(state.commercialAgendaValidationStart))} al ${escapeHtml(formatDate(state.commercialAgendaValidationEnd))}</p></div><p>Generado ${escapeHtml(new Date().toLocaleString("es-SV"))}</p></header><section class="summary"><div><span>Eventos</span><strong>${rows.length}</strong></div><div><span>Validados</span><strong>${reviewed}</strong></div><div><span>Efectivos</span><strong>${effective}</strong></div><div><span>Pendientes</span><strong>${rows.length-reviewed}</strong></div></section><table><thead><tr><th>Fecha</th><th>Horario</th><th>Vendedor</th><th>Cliente</th><th>Actividad</th><th>Comentario</th><th>Validación</th><th>Validado por</th></tr></thead><tbody>${rows.map(({item,event})=>{const v=event.validation||{};const status=v.validatedAt?(v.effective?"Efectiva":"No efectiva"):"Pendiente";return `<tr><td>${escapeHtml(formatDate(event.date))}</td><td>${escapeHtml(commercialAgendaTimeLabel(event.startTime))}–${escapeHtml(commercialAgendaTimeLabel(event.endTime))}</td><td>${escapeHtml(item.seller||"—")}</td><td>${escapeHtml(event.prospect||"—")}</td><td>${escapeHtml(event.activity||"—")}</td><td>${escapeHtml(event.comment||event.result||"Sin comentario")}</td><td class="${v.validatedAt?(v.effective?"yes":"no"):"pending"}">${status}</td><td>${escapeHtml(v.validatedBy||"—")}${v.validatedAt?`<br><small>${escapeHtml(new Date(v.validatedAt).toLocaleString("es-SV"))}</small>`:""}</td></tr>`;}).join("")}</tbody></table><footer>Reporte emitido por el Sistema Gerencial KONFI con la información registrada al momento de su generación.</footer><div class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button></div></body></html>`); popup.document.close();
}
function renderCommercialAgenda() {
  ...
  const rows = state.commercialAgenda.flatMap((item) => commercialAgendaItemEvents(item).map((event) => ({ item, event }))).filter(({ item, event }) => {
    ...
    return terms.every((term) => haystack.includes(term));
  }).sort((a,b) => `${b.event.date} ${b.event.startTime}`.localeCompare(`${a.event.date} ${a.event.startTime}`));
  const managementAccess = canViewCommercialAgendaManagement();
  if (!managementAccess) state.commercialAgendaView = "list";
  ...
  const activeView=state.commercialAgendaView==="management"&&managementAccess?renderCommercialAgendaManagement():state.commercialAgendaView==="validation"&&managementAccess?renderCommercialAgendaValidation():listView;
  return `<section class="commercial-agenda commercial-agenda-flat"><div class="commercial-agenda-view-tabs" role="tablist"><button type="button" role="tab" aria-selected="${state.commercialAgendaView === "list"}" class="${state.commercialAgendaView === "list" ? "active" : ""}" data-agenda-view="list">Listado</button>${managementAccess ? `<button type="button" role="tab" aria-selected="${state.commercialAgendaView === "management"}" class="${state.commercialAgendaView === "management" ? "active" : ""}" data-agenda-view="management">Cronograma</button><button type="button" role="tab" aria-selected="${state.commercialAgendaView === "validation"}" class="${state.commercialAgendaView === "validation" ? "active" : ""}" data-agenda-view="validation">Validación</button>` : ""}</div><div class="commercial-agenda-toolbar">${state.commercialAgendaView === "list" ? `<label class="commercial-agenda-search"><span aria-hidden="true">⌕</span><input type="search" data-agenda-search value="${escapeHtml(state.commercialAgendaQuery)}" placeholder="Buscar fecha, vendedor, cliente, actividad, descripción o comentario..." autocomplete="off"></label>` : `<span></span>`}${state.commercialAgendaView!=="validation"?`<button type="button" data-commercial-agenda-new><span aria-hidden="true">+</span> Nueva actividad</button>`:""}</div>${activeView}</section>`;
}
...
function wireCommercialAgenda(){
  ...
  opportunityTable.querySelector("[data-agenda-validation-start]")?.addEventListener("change",(event)=>{state.commercialAgendaValidationStart=event.target.value||`${todayISO().slice(0,8)}01`;if(state.commercialAgendaValidationEnd<state.commercialAgendaValidationStart)state.commercialAgendaValidationEnd=state.commercialAgendaValidationStart;renderCommercialSubmenu(areas.comercializacion);});
  opportunityTable.querySelector("[data-agenda-validation-end]")?.addEventListener("change",(event)=>{state.commercialAgendaValidationEnd=event.target.value||todayISO();if(state.commercialAgendaValidationStart>state.commercialAgendaValidationEnd)state.commercialAgendaValidationStart=state.commercialAgendaValidationEnd;renderCommercialSubmenu(areas.comercializacion);});
  opportunityTable.querySelector("[data-agenda-validation-report]")?.addEventListener("click",printCommercialAgendaValidationReport);
  opportunityTable.querySelectorAll("[data-agenda-validation-event]").forEach((input)=>input.addEventListener("change",async(event)=>{const control=event.currentTarget;control.disabled=true;try{const response=await apiJson("/api/commercial-agenda/validation",{method:"PATCH",body:JSON.stringify({eventId:control.dataset.agendaValidationEvent,effective:control.checked})});state.commercialAgenda=response.items||state.commercialAgenda;renderCommercialSubmenu(areas.comercializacion);}catch(error){control.checked=!control.checked;control.disabled=false;alert(error.message||"No se pudo validar el evento.");}}));
...
function renderCommercialSubmenu(area) {
  ...
  if (state.activeArea === "financiera" && submenu.key === "disponibilidad") {
    commercialPanel.classList.add("bank-availability-mode");
    newOpportunityBtn.classList.add("hidden"); newRiskBtn.classList.add("hidden"); newManagementRequestBtn.classList.add("hidden"); goalsMatrixBtn.classList.add("hidden");
