...
function openPendingExpensesCrud() {
  ...
  const render = () => {
    ...
    dialog.querySelector("[data-expense-settle]")?.insertAdjacentHTML("beforebegin", `<button type="button" data-expense-delete-selected ${selectedIds.size ? "" : "disabled"} style="border-color:#a9546c;background:#6b3046;color:#ffd5df">Eliminar seleccionados (${selectedIds.size})</button>`);
    ...
    dialog.querySelector("[data-expense-settle]")?.addEventListener("click", () => openPendingExpenseSettlement([...selectedIds], dialog));
