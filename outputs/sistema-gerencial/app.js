...
function commercialAgendaItemEvents(item) {
  ...
  return [{ id:`${item.id}-event`, date:item.date || item.startDate, prospect:item.prospect || "", startTime:item.startTime || "07:00", endTime:item.endTime || "08:00", activity:item.activity || commercialAgendaActivities[0], result:item.result || "" }];
}
...
function renderCommercialAgenda() {
  ...
  const rows = state.commercialAgenda.flatMap((item) => commercialAgendaItemEvents(item).map((event) => ({ item, event }))).filter(({ item, event }) => {
    ...
    const haystack = normalizeKey([event.date, formatDate(event.date), relativeDate, event.startTime, event.endTime, commercialAgendaTimeLabel(event.startTime), commercialAgendaTimeLabel(event.endTime), item.seller, event.prospect, event.activity, item.description || item.objective, event.result || item.result || "pendiente"].join(" "));
    return terms.every((term) => haystack.includes(term));
  ...
  const listView = `<div class="commercial-agenda-table commercial-agenda-flat-table"><table><thead><tr><th>Fecha</th><th>Horario</th><th>Vendedor</th><th>Cliente / prospecto</th><th>Actividad</th><th>Descripción</th><th>Resultado</th><th>Acciones</th></tr></thead><tbody>${rows.map(({item,event}) => `<tr><td><strong>${escapeHtml(formatDate(event.date))}</strong></td><td><strong>${escapeHtml(commercialAgendaTimeLabel(event.startTime))}</strong><small>${escapeHtml(commercialAgendaTimeLabel(event.endTime))}</small></td><td>${escapeHtml(item.seller || "—")}</td><td><strong>${escapeHtml(event.prospect || item.prospect || "—")}</strong></td><td>${escapeHtml(event.activity)}</td><td>${escapeHtml(item.description || item.objective || "—")}</td><td>${escapeHtml(event.result || item.result || "Pendiente")}</td><td><button type="button" data-commercial-agenda-edit="${escapeHtml(item.id)}" title="Editar agenda" aria-label="Editar agenda">✎</button><button class="danger" type="button" data-commercial-agenda-delete="${escapeHtml(item.id)}" title="Eliminar agenda completa" aria-label="Eliminar agenda completa">⌫</button></td></tr>`).join("") || `<tr><td colspan="8" class="empty-state">${terms.length ? "No hay coincidencias para esta búsqueda." : "No hay actividades registradas."}</td></tr>`}</tbody></table></div><small class="commercial-agenda-result-count">${rows.length} ${rows.length === 1 ? "evento encontrado" : "eventos encontrados"}</small>`;
  return `<section class="commercial-agenda commercial-agenda-flat"><div class="commercial-agenda-view-tabs"><button type="button" class="${state.commercialAgendaView === "list" ? "active" : ""}" data-agenda-view="list">Listado</button>${managementAccess ? `<button type="button" class="${state.commercialAgendaView === "management" ? "active" : ""}" data-agenda-view="management">Vista gerencial</button>` : ""}</div><div class="commercial-agenda-toolbar">${state.commercialAgendaView === "list" ? `<label class="commercial-agenda-search"><span aria-hidden="true">⌕</span><input type="search" data-agenda-search value="${escapeHtml(state.commercialAgendaQuery)}" placeholder="Buscar fecha, vendedor, cliente, actividad, descripción, resultado..." autocomplete="off"></label>` : `<span></span>`}<button type="button" data-commercial-agenda-new><span aria-hidden="true">+</span> Nueva actividad</button></div>${state.commercialAgendaView === "management" && managementAccess ? renderCommercialAgendaManagement() : listView}</section>`;
}
function openCommercialAgendaEditor(item = {}) {
  ...
  dialog.querySelector("[data-agenda-add-event]").addEventListener("click", () => { eventsContainer.insertAdjacentHTML("beforeend", eventMarkup({ date:dialog.querySelector('[name="startDate"]').value })); wireRemoveEvents(); }); wireRemoveEvents();
  dialog.querySelector("form").addEventListener("submit", async (event)=>{ event.preventDefault(); const values=Object.fromEntries(new FormData(event.currentTarget)); if(values.endDate < values.startDate) return alert("La fecha final no puede ser anterior a la fecha inicial."); const agendaEvents=[...eventsContainer.querySelectorAll("[data-agenda-event]")].map((row)=>({id:crypto.randomUUID(),date:row.querySelector("[data-event-date]").value,prospect:row.querySelector("[data-event-prospect]").value.trim(),activity:row.querySelector("[data-event-activity]").value,startTime:row.querySelector("[data-event-start]").value,endTime:row.querySelector("[data-event-end]").value,result:row.querySelector("[data-event-result]").value.trim()})); const invalidDate=agendaEvents.find((agendaEvent)=>agendaEvent.date<values.startDate||agendaEvent.date>values.endDate); if(invalidDate)return alert("Todas las fechas de los eventos deben estar dentro del rango de la agenda."); const validShift=(start,end)=>((start>="07:00"&&end<="12:00")||(start>="13:00"&&end<="17:00"))&&start<end; if(agendaEvents.some((agendaEvent)=>!validShift(agendaEvent.startTime,agendaEvent.endTime)))return alert("Cada evento debe estar dentro de 7:00 a. m.–12:00 m. o 1:00–5:00 p. m., sin cruzar el mediodía."); const record={id:item.id||crypto.randomUUID(),...values,events:agendaEvents}; const index=state.commercialAgenda.findIndex((row)=>row.id===record.id); if(index>=0) state.commercialAgenda[index]=record; else state.commercialAgenda.unshift(record); await saveCommercialAgenda(); dialog.close(); renderCommercialSubmenu(areas.comercializacion); }); dialog.showModal();
}
function wireCommercialAgenda(){ opportunityTable.querySelector("[data-commercial-agenda-new]")?.addEventListener("click",()=>openCommercialAgendaEditor()); opportunityTable.querySelectorAll("[data-agenda-view]").forEach((button)=>button.addEventListener("click",()=>{state.commercialAgendaView=button.dataset.agendaView;renderCommercialSubmenu(areas.comercializacion);})); opportunityTable.querySelector("[data-agenda-management-date]")?.addEventListener("change",(event)=>{state.commercialAgendaDate=event.target.value||todayISO();renderCommercialSubmenu(areas.comercializacion);}); opportunityTable.querySelectorAll("[data-agenda-date-step]").forEach((button)=>button.addEventListener("click",()=>{const date=new Date(`${state.commercialAgendaDate || todayISO()}T12:00:00Z`);date.setUTCDate(date.getUTCDate()+Number(button.dataset.agendaDateStep));state.commercialAgendaDate=date.toISOString().slice(0,10);renderCommercialSubmenu(areas.comercializacion);})); const search=opportunityTable.querySelector("[data-agenda-search]"); search?.addEventListener("input",(event)=>{state.commercialAgendaQuery=event.target.value;renderCommercialSubmenu(areas.comercializacion);const next=opportunityTable.querySelector("[data-agenda-search]");next?.focus();next?.setSelectionRange(next.value.length,next.value.length);}); opportunityTable.querySelectorAll("[data-commercial-agenda-edit]").forEach(button=>button.addEventListener("click",()=>openCommercialAgendaEditor(state.commercialAgenda.find(item=>item.id===button.dataset.commercialAgendaEdit)))); opportunityTable.querySelectorAll("[data-commercial-agenda-delete]").forEach(button=>button.addEventListener("click",async()=>{const item=state.commercialAgenda.find(row=>row.id===button.dataset.commercialAgendaDelete);if(!item||!confirm("¿Eliminar esta agenda y todos sus eventos?"))return;state.commercialAgenda=state.commercialAgenda.filter(row=>row.id!==item.id);await saveCommercialAgenda();renderCommercialSubmenu(areas.comercializacion);})); }
