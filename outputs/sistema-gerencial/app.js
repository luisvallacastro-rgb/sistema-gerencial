...
function openPendingExpensesCrud() {
  ...
  const render = () => {
    ...
    dialog.querySelector("[data-expense-select-all]")?.addEventListener("change", (event) => { rows.forEach((item) => event.target.checked ? selectedIds.add(item.id) : selectedIds.delete(item.id)); render(); });
