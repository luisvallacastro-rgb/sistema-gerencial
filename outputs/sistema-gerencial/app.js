...
async function printBankAvailabilityReport() {
  ...
  const row = (label, key, className = "") => `<tr><th>${label}</th>${accounts.map((item) => `<td class="${className}">${key === "date" ? (item.date ? formatDate(item.date) : "—") : formatMoney(item[key] || 0)}</td>`).join("")}</tr>`;
  ...
  const details = accounts.map((item) => {
    ...
    const movementRows = movements.map((movement, index) => `<tr><td>${index + 1}</td><td>${movement.date ? formatDate(movement.date) : "—"}</td><td>${escapeHtml(movement.description || "Movimiento bancario")}</td><td>${escapeHtml(movement.reference || "—")}</td><td class="charge">${movement.charge ? formatMoney(movement.charge) : "—"}</td><td class="credit">${movement.credit ? formatMoney(movement.credit) : "—"}</td><td class="money">${formatMoney(movement.balance || 0)}</td></tr>`).join("");
    return `<section class="account-detail"><div class="account-title"><div><h2>${escapeHtml(item.bank)}</h2><span>${escapeHtml(item.account)} · Corte ${item.date ? formatDate(item.date) : "sin movimientos"}</span></div><strong>${formatMoney(item.previousBalance || 0)} − ${formatMoney(item.charges || 0)} + ${formatMoney(item.credits || 0)} = ${formatMoney(item.newBalance || 0)}</strong></div><table class="detail-table"><thead><tr><th>#</th><th>Fecha</th><th>Descripción</th><th>Referencia</th><th>Cargo</th><th>Abono</th><th>Saldo resultante</th></tr></thead><tbody>${movementRows || `<tr><td colspan="7">Sin movimientos en el corte.</td></tr>`}</tbody><tfoot><tr><th colspan="4">Totales del banco</th><th>${formatMoney(item.charges || 0)}</th><th>${formatMoney(item.credits || 0)}</th><th>${formatMoney(item.newBalance || 0)}</th></tr></tfoot></table></section>`;
  }).join("");
  popup.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte de disponibilidad bancaria</title><style>
    @page{size:landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#17233a;font:12px Arial,sans-serif;background:#eef2f7}.sheet{width:100%;max-width:1400px;margin:20px auto;padding:28px;background:#fff}header{display:flex;justify-content:space-between;align-items:end;border-bottom:3px solid #28a98c;padding-bottom:14px;margin-bottom:22px}h1{margin:3px 0;font-size:27px}header p{margin:0;color:#64748b}.status{color:#147d69;font-weight:700}.matrix,.detail-table{width:100%;border-collapse:collapse;table-layout:fixed}.matrix th,.matrix td,.detail-table th,.detail-table td{border:1px solid #cbd5e1;padding:10px 8px;text-align:right}.matrix thead th,.detail-table thead th{background:#172f51;color:#fff;text-align:center}.matrix thead th:first-child{width:155px}.matrix thead strong,.matrix thead small{display:block}.matrix thead small{margin-top:4px;color:#cbd5e1;font-size:9px}.matrix tbody th{text-align:left;background:#f8fafc}.money{font-weight:700}.credit{color:#087f5b}.charge{color:#b42318}.matrix tfoot th,.matrix tfoot td{background:#e8f5f1;font-size:14px;font-weight:800}.formula{margin:16px 0 24px;color:#475569}.account-detail{margin-top:18px;break-inside:avoid-page}.account-title{display:flex;align-items:end;justify-content:space-between;gap:20px;padding:10px 12px;background:#edf5f3;border-left:4px solid #28a98c}.account-title h2{margin:0 0 2px;font-size:17px}.account-title span{color:#64748b}.account-title strong{font-size:12px;color:#173b62}.detail-table{font-size:10px}.detail-table th:nth-child(1){width:4%}.detail-table th:nth-child(2){width:10%}.detail-table th:nth-child(3){width:34%}.detail-table th:nth-child(4){width:16%}.detail-table th:nth-child(n+5){width:12%}.detail-table td:nth-child(1),.detail-table td:nth-child(2),.detail-table td:nth-child(3),.detail-table td:nth-child(4){text-align:left}.detail-table tfoot th,.detail-table tfoot td{background:#f1f5f9;font-weight:800}.actions{position:fixed;right:18px;bottom:18px;display:flex;gap:8px}.actions button{padding:11px 15px;border:0;border-radius:8px;background:#173b62;color:#fff;font-weight:700}.actions button:first-child{background:#16866d}@media print{body{background:#fff}.sheet{margin:0;padding:0;max-width:none}.actions{display:none}}
  </style></head><body><main class="sheet"><header><div><p>FINANCIERA · DISPONIBILIDAD BANCARIA</p><h1>Reporte de actualización de saldos</h1><p>Generado ${escapeHtml(new Date().toLocaleString("es-SV"))}</p></div><div class="status">${escapeHtml(status)}</div></header><table class="matrix"><thead><tr><th>Concepto</th>${headings}<th>Total consolidado</th></tr></thead><tbody>${row("Fecha de actualización", "date")} ${row("Saldo anterior", "previousBalance", "money")} ${row("(−) Cargos", "charges", "money charge")} ${row("(+) Abonos", "credits", "money credit")}</tbody><tfoot><tr><th>Saldo nuevo</th>${accounts.map((item) => `<td>${formatMoney(item.newBalance || 0)}</td>`).join("")}<td>${formatMoney(report.newTotal || 0)}</td></tr></tfoot></table><p class="formula"><b>Comprobación:</b> saldo anterior − cargos + abonos = saldo nuevo. Totales del corte: ${formatMoney(report.previousTotal || 0)} − ${formatMoney(report.chargesTotal || 0)} + ${formatMoney(report.creditsTotal || 0)} = <b>${formatMoney(report.newTotal || 0)}</b>.</p>${details}</main><nav class="actions"><button onclick="window.print()">Imprimir / Guardar PDF</button><button onclick="window.close()">Cerrar</button></nav></body></html>`);
  popup.document.close();
}
