...
function printArchivedDerivedAvailability(archiveId) {
  const archive = (state.bankAvailabilityHistory || []).find((item) => item.id === archiveId);
  if (!archive) { alert("No se encontró la disponibilidad archivada."); return; }
  const data = archive.reportData || {}, signatures = archive.signatures || {};
  const logoUrl = new URL("assets/konfi-logo.png", window.location.href).href;
...
function renderCommercialAgenda() {
  const eventRows = state.commercialAgenda.flatMap((item) => commercialAgendaItemEvents(item).map((event) => ({ item, event })));
  const days = Object.entries(eventRows.reduce((grouped, row) => {
    const date = row.event.date || "sin-fecha";
    (grouped[date] ||= []).push(row);
    return grouped;
  }, {})).sort(([dateA], [dateB]) => dateB.localeCompare(dateA));
  const dayRows = days.map(([date, rows], dayIndex) => {
    const sellers = Object.entries(rows.reduce((grouped, row) => {
      const seller = row.item.seller || "Sin vendedor";
      (grouped[seller] ||= []).push(row);
      return grouped;
    }, {})).sort(([sellerA], [sellerB]) => sellerA.localeCompare(sellerB, "es"));
    return `<details class="commercial-agenda-day" ${dayIndex === 0 ? "open" : ""}><summary><span class="commercial-agenda-day-toggle" aria-hidden="true"></span><strong>${escapeHtml(date === "sin-fecha" ? "Sin fecha" : formatDate(date))}</strong><span>${rows.length} ${rows.length === 1 ? "evento" : "eventos"}</span><span>${sellers.length} ${sellers.length === 1 ? "vendedor" : "vendedores"}</span></summary><div class="commercial-agenda-day-detail">${sellers.map(([seller, sellerRows]) => `<section class="commercial-agenda-seller"><header><strong>${escapeHtml(seller)}</strong><span>${sellerRows.length} ${sellerRows.length === 1 ? "evento" : "eventos"}</span></header><div class="commercial-agenda-table"><table><thead><tr><th>Horario</th><th>Cliente / prospecto</th><th>Actividad</th><th>Descripción de agenda</th><th>Resultado</th><th>Acciones</th></tr></thead><tbody>${sellerRows.sort((a,b) => String(a.event.startTime).localeCompare(String(b.event.startTime))).map(({ item, event }) => `<tr><td><strong>${escapeHtml(commercialAgendaTimeLabel(event.startTime))}</strong><small>${escapeHtml(commercialAgendaTimeLabel(event.endTime))}</small></td><td><strong>${escapeHtml(event.prospect || item.prospect || "—")}</strong></td><td>${escapeHtml(event.activity)}</td><td>${escapeHtml(item.description || item.objective || "—")}</td><td>${escapeHtml(event.result || item.result || "Pendiente")}</td><td><button type="button" data-commercial-agenda-edit="${escapeHtml(item.id)}" title="Editar agenda" aria-label="Editar agenda">✎</button><button class="danger" type="button" data-commercial-agenda-delete="${escapeHtml(item.id)}" title="Eliminar agenda completa" aria-label="Eliminar agenda completa">⌫</button></td></tr>`).join("")}</tbody></table></div></section>`).join("")}</div></details>`;
  }).join("");
  const managementAccess = canViewCommercialAgendaManagement();
  if (!managementAccess) state.commercialAgendaView = "daily";
  return `<section class="commercial-agenda commercial-agenda-compact"><div class="commercial-agenda-toolbar"><nav><button type="button" class="${state.commercialAgendaView === "daily" ? "active" : ""}" data-agenda-view="daily">Vista diaria</button>${managementAccess ? `<button type="button" class="${state.commercialAgendaView === "management" ? "active" : ""}" data-agenda-view="management">Vista gerencial</button>` : ""}</nav><button type="button" data-commercial-agenda-new><span aria-hidden="true">+</span> Nueva actividad</button></div>${state.commercialAgendaView === "management" && managementAccess ? renderCommercialAgendaManagement() : `<div class="commercial-agenda-days">${dayRows || `<div class="commercial-agenda-empty">No hay actividades registradas.</div>`}</div>`}</section>`;
}
