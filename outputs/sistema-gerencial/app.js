...
function printOpportunityManagementReport(options = {}) {
  ...
  const probabilityWeight = (probability) => ({ caliente: .8, tibio: .55, frio: .3, congelado: .1 }[probability] || .2);
  const weighted = rows.reduce((sum, row) => sum + Number(row.item.amount || 0) * probabilityWeight(row.item.probability), 0);
  const sellerSubtotals = Object.values(rows.reduce((groups, row) => {
    const seller = row.item.seller || "Sin vendedor";
    const current = groups[seller] || { seller, count: 0, amount: 0, weighted: 0 };
    const opportunityAmount = Number(row.item.amount || 0);
    current.count += 1;
    current.amount += opportunityAmount;
    current.weighted += opportunityAmount * probabilityWeight(row.item.probability);
    groups[seller] = current;
    return groups;
  }, {})).sort((a, b) => b.amount - a.amount || a.seller.localeCompare(b.seller, "es"));
  const sellerSubtotalRows = sellerSubtotals.map((row) => `<tr><td>${escapeHtml(row.seller)}</td><td>${row.count}</td><td class="money">${formatMoney(row.amount)}</td><td class="money">${isClosed ? "N/A" : formatMoney(row.weighted)}</td></tr>`).join("");
  ...
  ${isClosed ? "" : `<p class="weighted-note"><strong>¿Qué significa pipeline ponderado?</strong> Es una estimación del valor probable: cada oportunidad se multiplica por su probabilidad según temperatura — Caliente 80%, Tibio 55%, Frío 30% y Congelado 10% (sin definir 20%). No representa ventas cerradas.</p>`}
  <section class="seller-subtotals"><h2>Subtotales por vendedor</h2><table><thead><tr><th>Vendedor</th><th>Oportunidades</th><th>Monto total</th><th>Pipeline ponderado</th></tr></thead><tbody>${sellerSubtotalRows || `<tr><td colspan="4">Sin registros</td></tr>`}</tbody><tfoot><tr><td>Total general</td><td>${rows.length}</td><td class="money">${formatMoney(amount)}</td><td class="money">${isClosed ? "N/A" : formatMoney(weighted)}</td></tr></tfoot></table></section>
