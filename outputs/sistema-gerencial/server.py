...
def is_odaliz_valencia_user(user):
    """Identify the only account allowed to manage every seller's agenda."""
    if not user:
        return False
    identity = crm_identity_key(" ".join(text(user.get(field)) for field in ("id", "name", "username", "email")))
    return (
        ("odaliz" in identity and "valencia" in identity)
        or crm_identity_key(user.get("username")) == "gerencia comercial"
        or text(user.get("email")).lower() == "gtecomercial.ayc@gmail.com"
    )
...
COMMERCIAL_AGENDA_ACTIVITIES = {
    "Trabajo en Oficina",
    "Seguimiento a Oportunidad",
    "Visita a Nuevo Prospecto",
    "Reunión Interna",
    "Prospectos Nuevos",
}
COMMERCIAL_AGENDA_LEGACY_ACTIVITIES = {
    "Mensaje WhatsApp", "Llamada Telefónica", "Correo Electrónico",
    "Visita Presencial", "Elaboración de pedido", "Ingreso de pedido",
    "Preparación de oferta", "Gestión de cobro",
}
COMMERCIAL_OPPORTUNITY_STAGES = [
...
COMMERCIAL_STAGE_PROBABILITY = {
    "Prospeccion": "congelado",
    "Contacto inicial": "frio",
    "Deteccion de necesidades": "tibio",
    "Presentacion de solucion": "tibio",
    "Manejo de objeciones": "tibio",
    "Cierre de ventas": "caliente",
...
def apply_commercial_agenda_opportunity_stage(result_items, clean_event, seller, actor_name):
    ...
    if not opportunity:
        return "La oportunidad seleccionada ya no existe", False
    if crm_identity_key(opportunity.get("seller")) != crm_identity_key(seller):
    ...
    existing_management = next((management for management in managements
        if isinstance(management, dict) and text(management.get("agendaEventId")) == text(clean_event.get("id"))), None)
    has_closure = any(
        not management.get("canceled")
        and text(management.get("stage")) in {"Cierre", "Cierre de ventas"}
        and text(management.get("result"))
        for management in managements if isinstance(management, dict)
    )
    if not existing_management and (opportunity.get("archived") or text(opportunity.get("status")).lower() in {"ganada", "perdida", "anulada"} or has_closure):
    ...
    active_before_change = sorted(
        (management for management in managements
            if isinstance(management, dict) and not management.get("canceled") and not management.get("notified")),
    ...
    if existing_management:
        existing_management.update({
            "date": clean_event.get("date"),
            "time": clean_event.get("startTime"),
            "stage": next_stage,
            "comment": note,
            "editedAt": datetime.now(ZoneInfo("America/El_Salvador")).isoformat(timespec="seconds"),
            "editedBy": actor_name,
        })
        ...
        managements.append({
            "id": management_id,
            "agendaEventId": text(clean_event.get("id")),
            "date": clean_event.get("date"),
            "time": clean_event.get("startTime"),
            "stage": next_stage,
            "comment": note,
    ...
    latest_stage = text(active_managements[-1].get("stage")) if active_managements else next_stage
    if latest_stage == "Cierre":
        latest_stage = "Cierre de ventas"
    opportunity["stage"] = latest_stage
    opportunity["probability"] = COMMERCIAL_STAGE_PROBABILITY.get(latest_stage, opportunity.get("probability"))
    clean_event["stageManagementId"] = management_id
    clean_event["stageAppliedAt"] = datetime.now(ZoneInfo("America/El_Salvador")).isoformat(timespec="seconds")
    return None, True
...
def zero_2025_accounts_receivable_balances_once(conn):
    """Close only receivables due in 2025, preserving documents and payment history."""
    migration_key = "maintenance.accounts-receivable-zero-2025.2026-09-16.v1"
    if conn.execute("SELECT 1 FROM app_state WHERE key = ?", (migration_key,)).fetchone():
        return
    ...
    summary = conn.execute("""
        SELECT COUNT(*) AS record_count,
               COALESCE(SUM(balance), 0) AS previous_balance
        FROM accounts_receivable
        WHERE substr(due_date, 1, 4) = '2025'
          AND ABS(COALESCE(balance, 0)) > 0.000001
    """).fetchone()
    previous_balances = [dict(row) for row in conn.execute("""
        SELECT id, balance
        FROM accounts_receivable
        WHERE substr(due_date, 1, 4) = '2025'
          AND ABS(COALESCE(balance, 0)) > 0.000001
    ...
    conn.execute(
        ...
        (migration_key, json.dumps({
            ...
            "updatedRecords": int(summary["record_count"] or 0),
            "previousBalance": round(float(summary["previous_balance"] or 0), 2),
            "previousBalancesById": previous_balances,
            "newBalance": 0,
            "appliedAt": updated_at,
        }, ensure_ascii=False)),
    )
...
def archive_bank_availability(conn):
    ...
    for item in availability["accounts"]:
        ...
        if not latest:
            continue
        cutoff = {"date": latest["record_date"], "sequence": int(latest["sequence"] or 0), "balance": round(float(latest["balance"] or 0), 2)}
        cutoffs[item["id"]] = cutoff
        balances[item["id"]] = {"bank": item["bank"], "account": item["account"], "statementDate": cutoff["date"], "sequence": cutoff["sequence"], "balance": cutoff["balance"]}
    conn.execute(
...
def bank_report_baseline(conn, account_id, latest_sequence):
    ...
    return {"date": latest["record_date"], "sequence": int(latest["sequence"] or 0), "balance": round(float(latest["balance"] or 0), 2)} if latest else None
...
def init_db():
    with connect() as conn:
        ...
        zero_2025_accounts_receivable_balances_once(conn)
...
<footer>Entorno separado de producción · Solo red de oficina</footer></main></body></html>'''.encode("utf-8")
    ...
    def handle_api_post(self):
        ...
        if self.path == "/api/bank-availability/archive":
            if not self.require_permission("financiera:disponibilidad"):
                return
            with connect() as conn:
    ...
    def handle_api_put(self):
        ...
        if self.path == "/api/commercial-agenda":
            ...
            stored_events = {
                text(event.get("id")): event
                for stored_item in stored_items if isinstance(stored_item, dict)
                for event in stored_item.get("events", []) if isinstance(event, dict) and text(event.get("id"))
            }
